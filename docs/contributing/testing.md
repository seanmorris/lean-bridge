...
JavaScript uses `number` for floats and fixed-width integers through 32 bits, and `bigint` for 64-bit integers, `Nat` and `Int`. Integer-valued float inputs are valid JavaScript numbers; booleans are not integers. Generated npm exports reject wrong types and out-of-range inputs with `TypeError`. Both Node adapters check recovery after each rejection.

Reports appear in `build/type-corpus/`, named for the selected profiles: `python.json`, `ruby.json`, `perl.json`, `python-ruby.json`, `perl-python-ruby.json` or `node-javascript-node-typescript.json`. They record the input catalog, declaration checks, per-case observations, archive, runtime, binding IR, dependency, source and oracle identities. Perl also records its separate runtime archive and interpreter ABI. Node records both npm archives; TypeScript records its compiler, consumer source and generated declaration hashes. Every report lists all 17 consumer profiles and both source paths. Missing adapters, unsupported or unexecuted cases and the reviewed-IR path remain gaps. These scoped cases do not change the [type-support inventory](../reference/types.md).

CI requires Python, Ruby, both Node adapters and all four Perl configurations. Each job uploads its report as `type-corpus-python-<commit>`, `type-corpus-ruby-<commit>`, `type-corpus-node-<commit>` or `type-corpus-perl-<configuration>-<commit>`. A failed corpus run or missing artifact fails the corresponding consumer gate.
