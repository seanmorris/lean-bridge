...
Options, results and nested binary products can target npm, C, C++, Python, Rust, C#, Java and Kotlin. See the [conversion tables](reference/types.md) for each language's mappings and tested positions. npm copied containers and primitive callables currently require separate components.
