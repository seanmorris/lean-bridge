...
Task 1219 now has [compiled npm nested arrays](../evidence/npm-arrays-20260920.md) across all nineteen primitive element types. The versioned transport validates descriptors against Binding IR, bounds recursive copies and clears partial output. Node, strict TypeScript, browser pages, React and workers exercise installed archives on both source paths. Tuples, explicit Option tags and Except branches have a [staged codec](../evidence/npm-copied-transport-20260920.md), but are not yet admitted by the compiler. [Named copied records](../evidence/npm-records-20260920.md) now compile through Lean-generated constructors and field accessors. Variants and recursive copied types remain later steps in the same task.
