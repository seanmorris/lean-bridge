...
The combined builder compiles each profile once and stages its archives privately. Source drift, mismatched APIs, failure or cancellation prevents the final directory from appearing. Ordinary builds accept npm, CPAN, C, C++ and NuGet together when every selected profile admits the API. C/C++ and C# support all 16 copied primitives, arrays and acyclic records in concrete, pure function signatures. These targets reuse the compiled native component; generated record accessors keep its object layout private. C# wraps the shared C adapter with private scoped conversions and compiled .NET 8 assemblies. Arrays and records can share one native build with CPAN but cannot yet target npm. Resources and callbacks still require CPAN-only builds. Generic JVM/Ruby and WIT/WASI adapters remain subsequent slices of task 1216.
