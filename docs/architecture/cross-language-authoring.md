...
The combined builder compiles each profile once and stages its archives privately. Source drift, mismatched APIs, failure or cancellation prevents the final directory from appearing. Ordinary builds accept npm, CPAN, C, C++, NuGet, Maven and RubyGems together when every selected profile admits the API. C/C++, C#, Java/Kotlin and Ruby support all 16 copied primitives, arrays and acyclic records in concrete, pure function signatures. These targets reuse the compiled native component; generated record accessors keep its object layout private. C#, Java and Ruby wrap the shared C adapter with private scoped conversions. NuGet and Maven contain compiled managed code; RubyGems contains Ruby sources and compiled native libraries without an extension build. Arrays and records can share one native build with CPAN but cannot yet target npm. Resources and callbacks still require CPAN-only builds. Generic WIT/WASI adapters remain a subsequent slice of task 1216.
