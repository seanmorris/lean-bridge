...
Prepared [C/C++ packages](../evidence/native-compounds-20260920.md) and
[Python wheels](../evidence/python-compounds-20260920.md) also compile options,
results and binary products on both source paths. Python uses explicit `Some`,
`Ok` and `Err` values, preserving Unit and nested options. The remaining compound
adapters are C#, Java, Kotlin, Ruby, Rust, Perl, native PHP, PHP-Wasm and WIT/WASI.
