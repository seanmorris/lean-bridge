...
The shared configuration supplies `modules`, `exports`, `specializations`, `resources`, `arities`, and target metadata validation. Source analysis and builds hash the configuration. npm and CPAN implement finite specialization within their existing type profiles; CPAN also implements resources, closure arities, and its module/version settings. npm packaging and publication use the sealed settings while preserving the compiled Lean identity. Other configured target settings fail when their projection does not implement them. Generic host dispatch, checked-constructor, ownership, and effect decisions remain part of the dependent type-family work.
