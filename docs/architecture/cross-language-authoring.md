...
The [ordinary Python adapter](../evidence/native-python-20260915.md) generates source-named functions, type stubs and frozen dataclasses in deterministic platform wheels. Consumers use exact Python integers and owned copied values. A synchronized in-memory loader verifies bundled library hashes and shares one compatible runtime across independently installed wheels. No shared installed file is owned by multiple distributions. Rust/Cargo, PHP, wider type coverage, the shared corpus and generic receipts remain open under the stages above.
