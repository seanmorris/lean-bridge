...
The implemented package gate regenerates the JavaScript, PHP, Python, C, and Rust projections twice, scans each public source, declaration or stub, export map, and package README, then compares every generated file with a reviewed SHA-256 report. A changed public export, generated type, capability gap, generator version, documentation byte, or implementation byte blocks the check until the report receives an intentional review update.

The semantic parity gate compiles one language-neutral contract before it invokes a backend. That contract resolves callable sites, finite generic instantiations, value versus identity representation, mutability, ownership and lifetimes, error behavior, result delivery, documentation, source identity, and assurance claims. Every generated package must identify the same component and Binding IR hash. The report records how each target projects values, resources, callbacks, errors, cleanup, and asynchronous delivery. JavaScript, Python, C, and Rust share one generic specialization compiler. PHP blocks generic publication until its backend consumes the same policy.
