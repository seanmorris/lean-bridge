...
The semantic parity gate compiles one language-neutral contract before it invokes a backend. That contract resolves callable sites, finite generic instantiations, value versus identity representation, mutability, ownership and lifetimes, error behavior, result delivery, documentation, source identity, and assurance claims. Every generated package must identify the same component and Binding IR hash. The report records how each target projects values, resources, callbacks, errors, cleanup, and asynchronous delivery. Generic specialization validation lives in one shared compiler used by all four backends.
