...
This path does not emit an unrestricted generic host function or alter the existing `lean-wasm.org/specializations` overload-dispatch contract. The adapter generator accepts the application only with matching compiler metadata, and target compilation reproduces the whole report before linking. Configured specializations remain unavailable in the native CPAN profile.
