...
| `/docs/publish/` | Release a library package | Local candidate, sandbox rehearsal, approved production publication, ecosystem uploads, consumer handoff, and recovery. |
| `/docs/contributing/` | Maintain the project or update this site | Documentation, demos, tests, release-pipeline development, and GitHub Pages deployment. |
...
| --- | --- |
| `site/` | React routes, shared shell, accessible controls, documentation layout, and build configuration. |
| `site/app/components/` | Shared site and documentation layouts, workbench ownership, proof controls, receipts, and benchmark panels. |
| `site/registry.mjs` | Page routes, audience groups, ordering, search aliases, and explicit mappings to canonical Markdown. |
| `site/reference/` | Reviewed prose templates for contract-generated reference pages under `docs/reference/`. |
...
| 1188 | Remaining eleven React demos | Complete. Sweep-and-prune, Dinic, and the final nine ports retain legacy aliases and pass interaction and lifecycle checks. |
...
The implementation lives in [site/](../../site/README.md). It uses explicit route modules for each canonical guide and all twelve workbenches. React owns the common shell and page structure. Scoped controllers preserve the existing grid, graph, canvas, and editor interactions. All twelve retain their raw artifact URLs; their former page URLs redirect to the corresponding React routes.

Tasks 1190 through 1192 delivered author, consumer, and publisher guides. VO1193 connects those guides to generated CLI/API/type/algorithm references, shared-runtime composition, ownership, Dijkstra and flood-fill walkthroughs, and adoption guidance. The concept backlog is now an [implemented content map](../../demos/CONTENT_PLAN.md). Exact prepared archives pass installed Node and browser examples; source ownership and expected results are recorded in the evidence. VO1195's large-Nat runtime defect was fixed in commit `425ce9d`; the new installed-package checks include 4096-bit values. VO1194 must complete the combined release checks, including the isolated Docker checkout-ownership finding, and retain a rollback artifact before handing a clean candidate to VO1145.
