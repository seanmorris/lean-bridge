...
Target `c` also compiles [structured callback and closure payloads](../publish/c.md#export-callbacks-and-closures): arrays, Lists, options, results, products, acyclic records, variants and transparent aliases. Their payloads keep copied ownership; host callbacks remain call-scoped borrows and returned closures remain explicit leases. Other target projections still require primitive callback arguments and results.
