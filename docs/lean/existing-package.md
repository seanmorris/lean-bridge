...
| CPAN resource argument; C or CPAN callback argument | `"borrow"`, `{ "scope": "call", "anchor": null }` |
| CPAN resource result; C or CPAN Lean closure result | `"lease"`, `{ "scope": "explicit", "anchor": null }` |

Perl and Python leases use the generated object's lifetime and `close` operation; Python also supports `with` blocks. C uses explicit `_dispose` functions. A call-scoped borrow does not let Lean retain a host callback after the call. See the closure examples for [C](../publish/c.md#export-callbacks-and-closures), [CPAN](../publish/cpan.md#export-a-specialized-closure) and [PyPI](../publish/pypi.md#export-callbacks-and-closures).
