...
| CPAN resource argument; C, CPAN, PyPI or RubyGems callback argument | `"borrow"`, `{ "scope": "call", "anchor": null }` |
| CPAN resource result; C, CPAN, PyPI or RubyGems Lean closure result | `"lease"`, `{ "scope": "explicit", "anchor": null }` |

Perl, Python and Ruby leases use the generated object's lifetime and `close` operation; Python supports `with` blocks; Ruby supports `with { |closure| ... }`. C uses explicit `_dispose` functions. A call-scoped borrow does not let Lean retain a host callback after the call. See the closure examples for [C](../publish/c.md#export-callbacks-and-closures), [CPAN](../publish/cpan.md#export-a-specialized-closure) [PyPI](../publish/pypi.md#export-callbacks-and-closures) and [RubyGems](../publish/rubygems.md#export-callbacks-and-closures).
