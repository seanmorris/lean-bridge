...
Perl leases use the generated object's lifetime and `close` operation; C uses explicit `_dispose` functions. A call-scoped borrow does not let Lean retain a host callback after the call. See the closure examples for [C](../publish/c.md#export-callbacks-and-closures) and [CPAN](../publish/cpan.md#export-a-specialized-closure).
