...
| `reviewed-ir-build-unsupported` | An ordinary source project contains a supplied `.binding-ir.json` document. | `analyze` can validate that document; `build` cannot compile it yet. For a source build, keep reviewed documents outside the project and express supported decisions in `lean-bridge.exports.json`. Do not discard required constraints. |
