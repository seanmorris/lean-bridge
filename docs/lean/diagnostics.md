...
| `export-configuration-reviewed-ir` | Shared export decisions conflict with a reviewed Binding IR. | Select source roots with `modules`; keep export decisions in the review. Do not also set `exports`, `resources`, `arities`, `specializations` or `contracts`. |
| `reviewed-ir-build-unsupported` | The review lacks explicit source modules or requests unsupported semantics. | Check the [reviewed-build requirements](existing-package.md#compile-a-reviewed-contract). `analyze` can still validate richer documents without compiling them. |
...
| `analysis-configuration-unsupported` | Public analysis cannot project configured resources or closure arities yet. | Use a native target that implements those choices. Reviewed Binding IR can describe richer APIs for analysis; reviewed builds accept copied values and CPAN's synchronous primitive callables. |
...
| `native-constructor-allocation-unsupported` | A compiler-emitted constructor allocation exceeds the pinned native allocator's limits or has nonconstant sizes. | Reduce the fields stored directly in that constructor, for example with an Array. See the [native allocation checks](../evidence/native-allocation-guard-20260922.md). |
