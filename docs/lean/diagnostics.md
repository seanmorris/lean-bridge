...
| `reviewed-ir-build-unsupported` | The review lacks explicit source modules, requests unsupported semantics, or targets npm/PHP-Wasm. | Check the [native reviewed-build requirements](existing-package.md#compile-a-reviewed-contract). `analyze` can still validate richer documents without compiling them. |
...
| `analysis-configuration-unsupported` | Public analysis cannot project configured resources or closure arities yet. | Use a native target that implements those choices. Reviewed Binding IR can describe the API for analysis, but is not a build input. |
