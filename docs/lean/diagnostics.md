...
| `analysis-configuration-unsupported` | Public analysis cannot project configured resources or closure arities yet. | Use a native target that implements those choices. Reviewed Binding IR can describe richer APIs for analysis; reviewed builds currently accept only the documented pure copied subset. |
