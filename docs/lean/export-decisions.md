...
Ordinary [PHP-Wasm builds](../publish/php.md#build-an-ordinary-php-wasm-package) compile those copied values for a separate wasm32 Zend adapter. `UInt32` and `Int64` use `BigInteger` on this host, alongside `UInt64`, `Nat` and `Int`. Generated npm descriptors register the shared runtime and component before PHP starts; the companion Composer ZIP supplies the same public PHP API. Startup loading runs in Node-hosted PHP 8.4. Browser-engine and lazy-loading acceptance remain open.
