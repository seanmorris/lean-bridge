...
An installed registry backend receives the verified plan, candidate root, authorization, manifest hash, abort signal, and progress callback. A handler created without a publisher returns `registry-publisher-unavailable` with ordered pending targets and keeps `externalRegistryWrites` false. The installed CLI supplies the npm adapter and checks production deployment approvals before credential access.
...
`createCliHandlers({ registryAdapters })` installs the [`transactional registry coordinator`](transactional-registry-release.md). It preflights every coordinate before the first write, persists each target transition, resumes partial releases by idempotency key, and returns registry-specific recovery guidance. Cross-registry commits are explicitly non-atomic. The installed CLI registers the npm adapter. Production writes also require `LEAN_BRIDGE_NPM_PRODUCTION_OPT_IN=publish-to-production`, credentials, and a signer policy; sandbox mode uses a separate registry endpoint.
