...
The value-frame compiler derives copied-record offsets, buffer triples, widths, limits, and JavaScript lowering/lifting loops from Binding IR. It emits the matching C struct used by the shared module. The resource lifecycle and pending-operation compilers derive ownership and cleanup plans from the same IR. A JavaScript conformance fixture exercises a generated Promise function through the shared pending domain. The C code that constructs and projects Lean runtime objects remains handwritten. Lean-compiled Promise settlement, iterators, callbacks, properties, overloads, generics, and error translation still require backend conformance fixtures.
