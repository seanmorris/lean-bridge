...
The generated `BigInteger` is a canonical-decimal value wrapper, not Brick Math integration. The older Alpha PHP-Wasm `int` API still has the upper-half `UInt32` issue tracked by VO1206. Its resource, callback and returned-function mappings require separate work. VO1218 remains open for the remaining profiles, positions and type decisions. No registry publication is part of this acceptance.
