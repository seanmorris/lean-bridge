...
Ordinary packages remain startup-only. First-call lazy loading, other browser engines, worker execution, browser Composer mounts, generic CLI receipt verification and prepared compiler-input distribution remain open. This milestone does not change production loaders or archive bytes, publish a registry package, or promote the separate Alpha/reviewed-IR type evidence. Installed type-inventory coverage remains 656 cells.
