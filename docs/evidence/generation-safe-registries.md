...
The host registry lives in the JavaScript runtime context shared by every library loader for one Emscripten module. It assigns the same live token when the bridge retains the same JavaScript object under the same nominal kind. Each retain acquires a lease. Each release removes one lease. The final release clears the strong reference and advances the slot generation. A token from another runtime cannot resolve against an empty or incompatible registry. WP5 exercises this kernel through internal probes. A Lean callback or host-capability export does not retain a JavaScript value yet. That integration belongs to the callback work package.
