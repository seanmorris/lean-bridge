...
The tests use injected registry outcomes and an ephemeral Ed25519 key. The installed CLI now includes a gated npm adapter; these tests do not establish a public registry release. Production deployments must complete the deployment-profile approvals, provide a separately distributed public policy and external signer, and explicitly opt in to npm publication. Other registries require additional reviewed adapters. See the [release pipeline controls](../../src/release/README.md#publication-and-receipts).
