# Evidence index

Evidence documents record commands, artifact identities, measured results, and limitations for the proof of concept. Architecture requirements remain in the [architecture index](../architecture/README.md). Consumer claims remain in the [versioned support contract](../consumer-support.v1.json).

## Author workflow and release

- [Lean project analysis](lean-project-analysis.md)
- [Zero-configuration acceptance](zero-configuration-acceptance.md)
- [Plain project package acceptance](plain-project-package-acceptance.md)
- [External component CLI](external-component-cli.md)
- [Native component engine](native-component-engine.md)
- [Docker component engine](docker-component-engine.md)
- [Component artifact manifest](component-artifact-manifest.md)
- [Reproducibility release gate](reproducibility-release-gate.md)
- [Publication attestation](publication-attestation.md)
- [Transactional registry release](transactional-registry-release.md)
- [Release receipt](release-receipt.md)

## Consumer packages

- [npm package](npm-package.md)
- [Browser bundler acceptance](browser-bundler-acceptance.md)
- [Browser package acceptance](browser-package-acceptance.md)
- [Native PHP release](native-php-release-package.md)
- [PHP-Wasm release](php-wasm-release-package.md)
- [PHP release gate](php-release-gate.md)
- [PyPI package](pypi-package.md)
- [Cargo package](cargo-package.md)
- [C and C++ packages](c-family-package.md)
- [Native consumer acceptance](native-consumer-acceptance.md)
- [WIT projection](wit-projection.md)
- [WIT and WASI consumer acceptance](wasi-consumer-acceptance.md)
- [Generated package gate](generated-package-gate.md)

## Runtime and generated APIs

- [Direct-call conformance](direct-call-conformance.md)
- [Typed value frame](typed-value-frame.md)
- [Callback signature plan](callback-signature-plan.md)
- [Pending operation state machine](pending-operation-state-machine.md)
- [Generation-safe registries](generation-safe-registries.md)
- [Cross-language semantic parity](cross-language-semantic-parity.md)
- [PHP transport parity](php-transport-parity.md)
- [PHP-Wasm shared runtime composition](php-wasm-shared-runtime-composition.md)

## Performance

- [Downstream consumer performance, 12 August 2026](downstream-consumer-performance-20260812.md)
- [Time-to-package raw record](time-to-package-20260811.json)
- [Generated native call overhead](native-call-overhead.md)
- [Library scaling](library-scaling.md)
- [PHP transport performance](php-transport-performance.md)
- [Lifecycle stability](lifecycle-stability.md)
- [Performance methodology](performance-methodology.md)
- [Performance workloads](performance-workloads.md)
- [Performance reproducibility](performance-reproducibility.md)
- [Complete CI evidence contract](performance-ci.md)
