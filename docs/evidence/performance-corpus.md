...
The [shared-runtime spatial performance harness](spatial-performance-harness.md) now executes these public generated APIs across lazy, startup, final-static, and isolated-runtime profiles. It rejects a run unless correctness and ownership checks pass first. The measurement methodology and release threshold policy remain separate work.
