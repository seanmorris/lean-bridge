...
BUILD_DIR="$REPOSITORY_ROOT/build/demos/lean-tarjan"
...
MODULES=(TarjanCore TarjanReachability TarjanCertificate TarjanChecks TarjanTotal Tarjan)
...
em++ "${OBJECTS[@]}" "$BUILD_DIR/bridge.o" \
  ...
  -sEXPORTED_FUNCTIONS=_lean_tarjan_runtime_init,_lean_tarjan_prepare_c,_lean_tarjan_run,_lean_tarjan_release,_malloc,_free \
  -Wl,--no-entry -o "$OUTPUT_DIR/lean-tarjan.mjs"
wasm-tools validate --features all "$OUTPUT_DIR/lean-tarjan.wasm"
printf 'Built the proven Lean SCC browser module in %s\n' "$OUTPUT_DIR"
