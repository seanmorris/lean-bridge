/**
 * Scoped tarjan editor; React owns the surrounding page and controller lifetime.
 *
 * @file
 */
...
export const mountWorkbench = async (root, scope, saved = {}) => {
	const { initRuntime, prepareGraph } = scope.runtime(runtimeModule);
	const byId = id => root.querySelector("#" + id);
	const canvas = byId("graph-canvas");
	const svg = byId("graph-svg");
	const layer = byId("node-layer");
	let scene = saved.scene ?? createPreset();
	let partition = null;
	let selectedId = saved.selectedId ?? 1;
	let groupAnchor = saved.groupAnchor ?? null;
	let collapsed = saved.collapsed ?? false;
	let runtimeReady = false;
	let revision = 0;
	let solveFrame = 0;
	let drag = null;
	const groupPositions = saved.groupPositions ?? new Map();
	scope.remember(() => ({
		scene, selectedId, groupAnchor, collapsed, groupPositions
		, preset: root.querySelector('[data-preset][aria-pressed="true"]')?.dataset.preset
	}));
...
{ runtimeReady = true; void solve(); }
	...
	const renderGroups = () => {
		...
		for(const group of partition.groups)
		{
			...
			scope.listen(button, "click", () => selectGroup(group.index));
			list.append(button);
		}
	...
	const renderEditor = () => {
		...
		for(const node of scene.nodes)
		{
			...
			scope.listen(button, "click", () => toggleEdge(selected.id, node.id));
	...
	scope.listen(canvas, "pointerup", endDrag);
	scope.listen(canvas, "pointercancel", endDrag);
	scope.listen(canvas, "lostpointercapture", () => { drag = null; });
	...
	scope.listen(byId("show-all"), "click", () => { groupAnchor = null; render(); });
	scope.listen(byId("collapse-groups"), "click", () => { collapsed = !collapsed; renderGraph(); });
	scope.listen(byId("toggle-feedback"), "click", () => toggleEdge(...scene.feedback));
	scope.listen(byId("module-name"), "input", event => {
		selectedNode().name = event.target.value || "Unnamed module";
		render();
	});
	scope.listen(byId("remove-module"), "click", () => {
	...
	scope.listen(byId("add-module"), "click", () => {
	...
	for(const button of root.querySelectorAll("[data-preset]"))
		scope.listen(button, "click", () => {
			...
			for(const choice of root.querySelectorAll("[data-preset]"))
				choice.setAttribute("aria-pressed", String(choice === button));
			scheduleSolve();
		});
	scope.resizeObserver(renderGraph).observe(canvas);
	if(saved.scene) for(const button of root.querySelectorAll("[data-preset]")) button.setAttribute("aria-pressed", String(button.dataset.preset === saved.preset));
	render();
