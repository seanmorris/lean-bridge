/**
 * Scoped a-star editor; React owns the surrounding page and controller lifetime.
 *
 * @file
 */
...
export const mountWorkbench = async (root, scope, saved = {}) => {
	const { initRuntime, prepareSearch } = scope.runtime(runtimeModule);
	...
	let terrain = saved.terrain ?? createTerrain();
	let strength = saved.strength ?? 100;
	let tool = saved.tool ?? "wall";
	...
	let cursor = null;
	let focusedMap = null;
	scope.remember(() => ({ terrain, strength, tool }));
	...
	scope.listen(globalThis, "pageshow", event => {
		if(event.persisted)
{ runtimeReady = true; void solveMaps(); }
	});
	...
	for(const map of maps)
	{
		...
		scope.listen(canvas, "pointerup", stopDrag);
		scope.listen(canvas, "pointercancel", stopDrag);
		...
		scope.listen(canvas, "focus", () => {
			focusedMap = canvas;
			cursor ??= terrain.start;
			drawMaps();
		});
		scope.listen(canvas, "blur", () => {
			focusedMap = null;
			if(!drag) cursor = null;
			drawMaps();
		});
	...
	for(const button of root.querySelectorAll("[data-tool]"))
		scope.listen(button, "click", () => chooseTool(button.dataset.tool));
	for(const button of root.querySelectorAll("[data-strength]"))
		scope.listen(button, "click", () => {
			strength = Number(button.dataset.strength);
			for(const choice of root.querySelectorAll("[data-strength]"))
	...
	const observer = scope.resizeObserver(drawMaps);
	for(const map of maps) observer.observe(map.canvas);
	byId("terrain-seed").value = terrain.seed;
	chooseTool(tool);
	for(const button of root.querySelectorAll("[data-strength]")) button.setAttribute("aria-pressed", String(Number(button.dataset.strength) === strength));
	byId("astar-estimate").textContent = `${strength}% estimate`;
	byId("heuristic-copy").textContent = strength === 0
		? "At 0%, A* uses no estimate and searches like Dijkstra."
		: `${strength}% uses ${strength === 50 ? "half the" : "the"} grid distance to the goal. Each step costs at least 1, so this estimate never overstates the remaining cost.`;
	drawMaps();
