...
BUILD_DIR="$REPOSITORY_ROOT/build/demos/lean-myers"
...
MODULES=(EditSpec EditProofs EditReference ArrayScriptCheck ScoreCertificate MyersCore EditCertificate Myers)
...
em++ "${OBJECTS[@]}" "$BUILD_DIR/bridge.o" \
  ...
  -sEXPORTED_FUNCTIONS=_lean_myers_runtime_init,_lean_myers_prepare_c,_lean_myers_run,_lean_myers_release,_malloc,_free \
  -Wl,--no-entry -o "$OUTPUT_DIR/lean-myers.mjs"
wasm-tools validate --features all "$OUTPUT_DIR/lean-myers.wasm"
printf 'Built the proven Lean shortest-edit browser module in %s\n' "$OUTPUT_DIR"
