...
BUILD_DIR="$REPOSITORY_ROOT/build/demos/lean-dinic"
...
MODULES=(FlowSpec FlowProofs FlowCertificate FlowPaths FlowAugment FlowDuality FlowReference DinicCore Dinic)
...
em++ "${OBJECTS[@]}" "$BUILD_DIR/bridge.o" \
  ...
  -sEXPORTED_FUNCTIONS=_lean_dinic_runtime_init,_lean_dinic_prepare_c,_lean_dinic_run,_lean_dinic_release,_malloc,_free \
  -Wl,--no-entry -o "$OUTPUT_DIR/lean-dinic.mjs"
wasm-tools validate --features all "$OUTPUT_DIR/lean-dinic.wasm"
printf 'Built the proven Lean max-flow browser module in %s\n' "$OUTPUT_DIR"
