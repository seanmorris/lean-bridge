...
BUILD_DIR="$REPOSITORY_ROOT/build/demos/lean-aho-corasick"
...
LEAN_PATH="$GENERATED_DIR${LEAN_PATH:+:$LEAN_PATH}" lean -R "$DEMO_ROOT" \
  -o "$GENERATED_DIR/AhoCorasickCore.olean" -c "$GENERATED_DIR/AhoCorasickCore.c" \
  "$DEMO_ROOT/AhoCorasickCore.lean"
LEAN_PATH="$GENERATED_DIR${LEAN_PATH:+:$LEAN_PATH}" lean -R "$DEMO_ROOT" \
  -o "$GENERATED_DIR/AhoCorasick.olean" "$DEMO_ROOT/AhoCorasick.lean"
...
emcc -O3 "${LEAN_WASM_PROFILE_CC_FLAGS[@]}" "${INCLUDES[@]}" \
  -c "$GENERATED_DIR/AhoCorasickCore.c" -o "$BUILD_DIR/AhoCorasickCore.o"
emcc -O3 "${LEAN_WASM_PROFILE_CC_FLAGS[@]}" "${INCLUDES[@]}" \
  -c "$DEMO_ROOT/bridge.c" -o "$BUILD_DIR/bridge.o"
em++ "$BUILD_DIR/AhoCorasickCore.o" "$BUILD_DIR/bridge.o" \
  ...
  -sEXPORTED_FUNCTIONS=_lean_aho_runtime_init,_lean_aho_prepare,_lean_aho_scan,_lean_aho_release,_malloc,_free \
  -Wl,--no-entry -o "$OUTPUT_DIR/lean-aho-corasick.mjs"

wasm-tools validate --features all "$OUTPUT_DIR/lean-aho-corasick.wasm"
printf 'Built the proven Lean Aho-Corasick browser module in %s\n' "$OUTPUT_DIR"
