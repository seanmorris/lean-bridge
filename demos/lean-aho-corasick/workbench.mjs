/**
 * Scoped aho-corasick editor; React owns the surrounding page and controller lifetime.
 *
 * @file
 */
...
export const mountWorkbench = async (root, scope, saved = {}) => {
	const { prepareMatcher, ready, unpackMatches } = scope.runtime(runtimeModule);
	...
	let patterns = [];
	let matches = [];
	let selectedPattern = saved.selectedPattern ?? null;
	let revision = 0;
	let updateTimer = 0;
	let scanTimer = 0;
	let alive = true;
	scope.remember(() => ({
		patterns: elements.patterns.value
		, input: elements.input.value
		, selectedPattern
		, scenario: root.querySelector('[data-scenario][aria-pressed="true"]')?.dataset.scenario
	}));
	...
	const render = () => {
		...
		elements.results.replaceChildren(...patterns.map((pattern, index) => {
			...
			scope.listen(button, "click", () => { selectedPattern = selectedPattern === index ? null : index; render(); });
			return button;
		}));
	};
	...
	const compileAndScan = async (restoreSelection = null) => {
		...
		try
		{
			...
			selectedPattern = restoreSelection !== null && restoreSelection < patterns.length ? restoreSelection : null;
			scan();
		}
		catch(error)
		{
	...
	const scheduleCompile = () => {
		revision++;
		scope.clearTimeout(updateTimer);
		scope.clearTimeout(scanTimer);
		clearMatcher();
		...
		updateTimer = scope.setTimeout(() => void compileAndScan(), 160);
	};
	const scheduleScan = () => { scope.clearTimeout(scanTimer); scanTimer = scope.setTimeout(scan, 55); };
	const loadScenario = name => {
		...
		for(const button of root.querySelectorAll("[data-scenario]")) button.setAttribute("aria-pressed", String(button.dataset.scenario === name));
		void compileAndScan();
	};

	for(const button of root.querySelectorAll("[data-scenario]")) scope.listen(button, "click", () => loadScenario(button.dataset.scenario));
	scope.listen(elements.patterns, "input", scheduleCompile);
	scope.listen(elements.input, "input", scheduleScan);
	scope.listen(elements.showAll, "click", () => { selectedPattern = null; render(); });
	...
	scope.listen(globalThis, "pageshow", event => {
		...
		{
			alive = true;
			void compileAndScan(selectedPattern);
		}
	});
	...
	{
		elements.patterns.value = saved.patterns;
		elements.input.value = saved.input;
		for(const button of root.querySelectorAll("[data-scenario]")) button.setAttribute("aria-pressed", String(button.dataset.scenario === saved.scenario));
		void compileAndScan(saved.selectedPattern);
	}
	else loadScenario("operations");
	try
	{ await ready(); elements.runtime.textContent = "Lean/Wasm ready"; }
	catch(error)
	{
		if(!scope.active) return; elements.runtime.textContent = "Lean/Wasm failed to load"; console.error(error); }
};
