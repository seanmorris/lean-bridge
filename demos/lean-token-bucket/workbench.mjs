/**
 * Scoped token-bucket editor; React owns the surrounding page and controller lifetime.
 *
 * @file
 */
...
export const mountWorkbench = async (root, scope, saved = {}) => {
	const { createBucket } = scope.runtime(runtimeModule);
	...
	const decisionPanel = root.querySelector(".decision-panel");
	const labControls = [...root.querySelectorAll(".bucket-workbench button,.bucket-workbench input")];
	...
	let bucket;
	let capacity = saved.capacity ?? EXAMPLE_CONFIG.capacity;
	let rate = saved.rate ?? EXAMPLE_CONFIG.rate;
	let cost = saved.cost ?? EXAMPLE_CONFIG.cost;
	let now = 0;
	let tokens = 0;
	let busy = true;
	let alive = true;
	...
	const snapshot = () => ({
		...
		, requestCount
		, selected
		, scenarioCopy: elements["scenario-copy"].textContent
	});
	scope.remember(() => bucket ? snapshot() : saved);
	...
	const tick = frameTime => {
		...
		if(playing) frame = scope.requestAnimationFrame(tick);
	};
	...
	const reset = async ({ initial = false, replay = false, restore } = {}) => {
		...
		try
		{
			const created = await createBucket({ capacity: capacity * CREDITS_PER_TOKEN, rate, now: restore?.now ?? 0 });
			...
			if(restore)
			{
				// Restore through the compiled API: debit a full bucket at the saved time.
				bucket.request(restore.now, capacity * CREDITS_PER_TOKEN - restore.tokens);
				({ tokens, timestamp: now } = bucket.snapshot());
				({ history, allowedCount, throttledCount, requestCount, selected } = restore);
			}
			...
			if(restore) elements["scenario-copy"].textContent = restore.scenarioCopy;
			renderScale();
			render();
			if(replay)
			{
		...
		{
			if(!scope.active) return; if(currentRevision === revision && alive) showError(error); }
	};
	...
	scope.listen(elements["send-request"], "click", () => send(1));
	for(const button of root.querySelectorAll("[data-burst]")) scope.listen(button, "click", () => send(Number(button.dataset.burst)));
	for(const button of root.querySelectorAll("[data-advance]")) scope.listen(button, "click", () => {
		if(busy) return;
		stopClock();
		example = null;
		try
	...
	scope.listen(elements["play-clock"], "click", () => {
		if(playing) stopClock();
		else startClock();
		renderBalance();
	});
	scope.listen(elements["reset-bucket"], "click", () => void reset());
	scope.listen(elements["replay-example"], "click", () => void reset({ replay: true }));
	for(const name of ["capacity", "rate"])
	{
		...
		scope.listen(elements[name], "change", () => {
			capacity = Number(elements.capacity.value);
			rate = Number(elements.rate.value);
			void reset();
		});
	}
	scope.listen(elements["request-cost"], "change", () => {
	...
	scope.listen(globalThis, "pageshow", event => {
		...
		{
			alive = true;
			void reset({ restore: snapshot() });
		}
	});

	mountBenchmark(scope);
	void reset(saved.history ? { restore: saved } : { initial: true });
};
