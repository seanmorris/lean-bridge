/**
 * Runs the interactive build graph through the checked Lean scheduler.
 *
 * @file
 */

import { prepareSort, ready, sortGraph } from "./runtime.mjs";
...
let tasks = healthyTasks();
let edges = healthyEdges();
let selected = 5;
let result = null;
let solveRevision = 0;
let drag = null;
...
byId("delete-task").addEventListener("click", () => {
...
new ResizeObserver(() => requestAnimationFrame(renderEdges)).observe(elements.canvas);
