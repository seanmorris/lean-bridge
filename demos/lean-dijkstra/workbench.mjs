/**
 * Scoped dijkstra editor; React owns the surrounding page and controller lifetime.
 *
 * @file
 */
...
export const mountWorkbench = async (root, scope, saved = {}) => {
	const { prepareShortestPath, ready, shortestPath } = scope.runtime(runtimeModule);
	const COLUMNS = 28;
	const ROWS = 18;
	...
	const visited = root.querySelector("#visited");
	...
	if(saved.walls)
	{
		walls.set(saved.walls);
		({ start, target, mode } = saved);
		userEdited = true;
	}
	scope.remember(() => ({ walls, start, target, mode }));
	...
	const animateInitialMaze = duration => new Promise(resolve => {
		...
		const nextFrame = async timestamp => {
			...
			render();
			await solve();
			if(timestamp - started < duration) scope.requestAnimationFrame(nextFrame);
			else
			{
				grid.classList.remove("maze-cycling");
				resolve();
		...
		scope.requestAnimationFrame(nextFrame);
	});

	const setMode = nextMode => {
		mode = nextMode;
		for(const button of root.querySelectorAll(".mode"))
		{
	...
	scope.listen(grid, "pointerdown", event => {
	...
	scope.listen(grid, "pointermove", event => {
	...
	for(const type of ["pointerup", "pointercancel", "lostpointercapture"]) scope.listen(grid, type, stopDrawing);
	scope.listen(grid, "keydown", event => {
		const cell = event.target.closest(".cell");
		if(!cell) return;
		const index = Number(cell.dataset.index);
	...
	for(const button of root.querySelectorAll(".mode"))
	{
		scope.listen(button, "click", () => setMode(button.dataset.mode));
	}

	scope.listen(root.querySelector("#clear"), "click", () => {
		userEdited = true;
		walls.fill(0);
		render();
		solve();
	...
	scope.listen(root.querySelector("#maze"), "click", () => {
		userEdited = true;
		installMaze();
		render();
		solve();
	...
	scope.listen(globalThis, "keydown", event => {
	...
	render();
	setMode(mode);
	const demoReady = ready().then(() => saved.walls || matchMedia("(prefers-reduced-motion: reduce)").matches ? solve() : animateInitialMaze(200));
	let benchmarkReady;
