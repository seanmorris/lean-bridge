/**
 * Presents Lean cache transitions as an interactive resource request timeline.
 *
 * @file
 */

import { createCache, ready } from "./runtime.mjs";
...
let cache;
let scenarioName = "working";
let requests = [...scenarios.working.requests];
let history = [];
let capacity = 3;
let busy = true;
let playing = false;
...
const renderTimeline = () => {
	...
	elements.timeline.replaceChildren(...requests.map((key, index) => {
		...
		button.addEventListener("click", () => void resetAt(index + 1).then(() => {
...
const render = () => {
	...
	for(const button of globalThis.document.querySelectorAll("[data-scenario]"))
	{
		button.disabled = busy;
...
elements.resources.replaceChildren(...resources.map(item => {
	...
	button.addEventListener("click", () => insertRequest(item.key));
	return button;
}));

elements.step.addEventListener("click", () => { stopPlayback(); step(); });
elements.run.addEventListener("click", () => {
	if(playing)
{ stopPlayback(); render(); }
	else startPlayback();
});
elements.replay.addEventListener("click", () => void resetAt(0, true));
elements.capacity.addEventListener("change", () => {
	capacity = Number(elements.capacity.value);
	void resetAt(history.length);
});
for(const button of globalThis.document.querySelectorAll("[data-scenario]")) button.addEventListener("click", () => {
