/**
 * Installed structured C callbacks and closures from both authoring paths.
 *
 * @file
 */
...
import { structuredCallableCConsumer } from "./helpers/structured-callable-c-consumer.mjs";
import { checkStructuredCallableFaults } from "./helpers/c-structured-callable-faults.mjs";

const enabled = process.env.LEAN_BRIDGE_C_STRUCTURED_CALLABLE_TEST === "1";
const signatures = ir => {
	const type = ref => {
...
test("installed C structured callbacks preserve mixed values, borrowed lifetimes and owned closures on both source paths", { skip: !enabled, timeout: 900_000 }, async t => {
	const reports = [], expected = signatures(structuredCallableReviewedIr());
	for(const path of ["ordinary-source", "reviewed-ir"])
	{
		const author = await mkdtemp(join(tmpdir(), "lean-bridge-c-structured-author-"));
		const consumer = await mkdtemp(join(tmpdir(), "lean-bridge-c-structured-consumer-"));
		...
		t.diagnostic(`${path}: compiling structured C callbacks and returned closures`);
		...
		const allocationFailures = await checkStructuredCallableFaults(outputRoot, join(author, "faults"), environment);
		...
		t.diagnostic(`${path}: installing relocated archives after removing author sources and builds`);
		const { command, ...observation } = await installCopiedConsumer({
			profile: "c", consumer, handoff
			, packages: receipt.packages, environment
			, fixture: { source: structuredCallableCConsumer, success: "structured-c" } });
		assert.ok(command.startsWith(consumer));
		reports.push({ profile: "c", path, ...observation, allocationFailures
			, signatures: expected
	...
	const reportPath = resolve(process.env.LEAN_BRIDGE_C_STRUCTURED_CALLABLE_REPORT ?? "build/structured-callables/c.json");
