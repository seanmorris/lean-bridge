/**
 * Bind list claims to installed wasm32 packages and separate Zend probes.
 *
 * @file
 */
...
import { phpWasmListConsumer, phpWasmListRequest } from "./helpers/php-wasm-list-fixture.mjs";
import { phpWasmDriverHashes, phpWasmIsolationFlags } from "./helpers/type-corpus-php-wasm-evidence.mjs";
import { zendListFaultIr, zendListFaultProvider, zendListFaultConsumer } from "./helpers/php-wasm-list-faults.mjs";
...
test("PHP-Wasm list evidence covers installed public APIs and exact loading arrangements", async () => {
	...
	for(const run of record.executions)
	{
		...
		{
			assert.equal(evidence.consumerSources[mode], sha256(phpWasmListConsumer(mode, run.path)));
			assert.equal(files[mode + ".php"].sha256, evidence.consumerSources[mode]);
		}
		for(const arrangement of ["embedded", "composer"])
		{
			assert.equal(evidence.requests[arrangement], sha256(phpWasmListRequest(arrangement)));
		...
		for(const execution of evidence.executions)
		{
			...
			assert.equal(result.checks, 86992); assert.equal(result.word_bits, 32); assert.equal(result.php, "8.4.1");
			assert.deepEqual(result.primitives.map(item => item.name).sort(), [...listPrimitives].sort());
			assert.ok(result.primitives.every(item => item.checks === 1288));
			assert.equal(result.api, "/vendor/lean-bridge-lists/wasm/src/Api.php");
	...
	const source = await readFile("tests/fixtures/list-consumers/php-wasm.php", "utf8");
