...
test("dedicated CI covers every consumer with Node 22 and pinned build paths", async () => {
  ...
  assert.equal(packageDocument.scripts["test:type-corpus:python"], "LEAN_BRIDGE_TYPE_CORPUS_PYTHON=1 node --test tests/type-corpus.test.mjs");
  ...
  assert.equal((workflow.match(/^\s*overwrite: true$/gm) ?? []).length, 8);
