...
test("dedicated CI covers every consumer with Node 22 and pinned build paths", async () => {
  ...
  assert.match(workflow, /node --test tests\/native-wit\.test\.mjs && LEAN_BRIDGE_WIT_CALLABLE_COMPONENT_TEST=1 node --test tests\/wit-callable-contract\.test\.mjs && LEAN_BRIDGE_WIT_CALLABLE_TEST=1 node --test tests\/wit-callables\.test\.mjs && LEAN_BRIDGE_WIT_COMPOUND_TEST=1 node --test tests\/wit-compounds\.test\.mjs tests\/wit-compound-contract\.test\.mjs tests\/wit-compound-conversions\.test\.mjs && LEAN_BRIDGE_WIT_LIST_TEST=1 node --test tests\/wit-lists\.test\.mjs tests\/wit-list-contract\.test\.mjs tests\/wit-list-conversions\.test\.mjs && LEAN_BRIDGE_WIT_ALIAS_TEST=1 node --test --test-concurrency=1 tests\/wit-aliases\.test\.mjs tests\/wit-alias-contract\.test\.mjs tests\/wit-alias-conversions\.test\.mjs && npm run test:type-corpus:wit-wasi/);
