/**
 * Installed Python compound values on independently specified build paths.
 *
 * @file
 */
...
const enabled = process.env.LEAN_BRIDGE_PYTHON_COMPOUND_TEST === "1";
...
test("installed Python compounds preserve copied values on both source paths", { skip: !enabled, timeout: 900_000 }, async t => {
	const reports = [], environment = nativeFixtureEnvironment(["python"]);
	for(const path of ["ordinary-source", "reviewed-ir"])
	{
		const author = await mkdtemp(join(tmpdir(), "lean-bridge-python-compound-author-"));
		const consumer = await mkdtemp(join(tmpdir(), "lean-bridge-python-compound-consumer-"));
		...
		await saveLakeFile(projectRoot, "lean-bridge.exports.json", canonicalJson({ schemaVersion: 1
			, modules: ["Compounds"]
			, targets: { pypi: { name: "compounds-api", version: "1.0.0" } }
		...
		t.diagnostic(`${path}: compiling Python compounds`);
		...
		const { command, ...observation } = await installCopiedConsumer({ profile: "python"
			...
			, fixture: { source: () => readFile("tests/fixtures/compound-consumers/python.py", "utf8"), success: "compound-python-ok" } });
		void command;
	...
	await saveLakeFile("build/compounds", "python.json", canonicalJson({ schemaVersion: 1, reports }));
});
