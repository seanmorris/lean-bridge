/**
 * Synthetic wasm32 variant ownership faults, separate from real Lean acceptance.
 *
 * @file
 */
...
test("wasm32 Zend variants clear all selected owners on failures and request bailouts", { skip: process.env.LEAN_BRIDGE_PHP_WASM_VARIANT_TEST !== "1", timeout: 240_000 }, async t => {
	const root = await mkdtemp(join(tmpdir(), "lean-bridge-zend-variant-fault-"));
	t.after(() => rm(root, { recursive: true, force: true }));
	const ir = zendVariantFaultIr(), files = generateCopiedPhpZendAdapter(ir);
	...
	const request = { functions: {}, families: [] };
	for(const fn of model.surface.functions.filter(fn => fn.field.startsWith("echo_") || fn.field === "signals"))
	{
		...
		if(copy.variant) request.families.push({ function: fn.field, constructors: copy.cases.length });
	}
	assert.equal(request.families.length, 7);
	...
	const provider = zendVariantFaultProvider(ir);
	...
	const consumer = await readFile("tests/fixtures/variant-consumers/php-wasm-faults.php", "utf8");
...
for(const mode of [7,8]) {
  stdout=''; stderr='';
  await php.run("<?php $beforeCalls=LeanVariants\\\\native_calls(); $beforeClears=LeanVariants\\\\output_clears(); LeanVariants\\\\configure("+mode+"); LeanVariants\\\\echo_buffers(new LeanVariants\\\\BuffersPair(LeanVariants\\\\Bytes::fromString('first'), LeanVariants\\\\Bytes::fromString('second')));");
  const status=await php.run("<?php LeanVariants\\\\configure(0); echo LeanVariants\\\\live_allocations(), ':', LeanVariants\\\\native_calls()-$beforeCalls, ':', LeanVariants\\\\output_clears()-$beforeClears, ':', LeanVariants\\\\echo_signal(new LeanVariants\\\\SignalIdle()) instanceof LeanVariants\\\\SignalIdle ? 'recovered' : 'failed';");
  if(status || stderr || stdout!=='0:1:1:recovered') throw new Error(JSON.stringify({status,stdout,stderr,mode}));
}
...
`);
	...
	for(const mode of ["0", "1"])
	{
		...
		assert.equal(new Set(result.constructors).size, 18);
		assert.equal(result.malformedTags, 7); assert.equal(result.malformedPayloads, 7);
		assert.equal(result.inactivePayloadCases, 8); assert.equal(result.partialInputs, 32);
		assert.equal(result.wireRejections, 92); assert.equal(result.bailoutRecovery, 2);
		assert.equal(result.oneClearPerNativeEntry, true);
		executions.push({ mode: mode === "1" ? "strict" : "weak", ...result });
	}
	const report = resolve("build/variants/php-wasm-zend-faults.json");
