/**
 * Installed wasm32 Lean arrays and records in offline npm and Composer deployments.
 *
 * @file
 */
...
import { phpWasmCollectionSettings, phpWasmCollectionConsumer, phpWasmCollectionRequest, phpWasmCollectionDocumentation } from "./helpers/php-wasm-collection-fixture.mjs";

const enabled = process.env.LEAN_BRIDGE_PHP_WASM_COLLECTION_TEST === "1";
const type = value => value.kind === "primitive" ? value.name
	: ["array", "list", "option"].includes(value.kind) ? { [value.kind]: type(value.element) }
...
test("installed PHP-Wasm arrays and records preserve copied values in Node and Chromium", { skip: !enabled, timeout: 1_800_000 }, async t => {
	const reports = [];
	for(const path of ["ordinary-source", "reviewed-ir"])
	{
		const author = await mkdtemp(join(tmpdir(), "lean-bridge-php-wasm-collection-author-"));
		const consumer = await mkdtemp(join(tmpdir(), "lean-bridge-php-wasm-collection-consumer-"));
		t.after(() => Promise.all([author, consumer].map(root => rm(root, { recursive: true, force: true }))));
		const projectRoot = join(author, "project"), outputRoot = join(author, "release"), handoff = join(consumer, "handoff");
		await writeCollectionProject(projectRoot);
		await saveLakeFile(projectRoot, "lean-bridge.exports.json", canonicalJson({ schemaVersion: 1
			, modules: ["Collections"]
			, targets: { "php-wasm": phpWasmCollectionSettings }
			, ...(path === "ordinary-source" ? { exports: collectionSignatures.map(entry => entry.name) } : {}) }));
		...
		t.diagnostic(`${path}: compiling the 35-export wasm32 collection library`);
		...
		const installed = await installedPhpWasmCorpus({ t
			...
			, sourcePath: path
			, fixture: { settings: phpWasmCollectionSettings
				, source: phpWasmCollectionConsumer
				, request: phpWasmCollectionRequest, removeHandoff: true } })
		...
		{
			assert.ok(execution.observation.checks > 190000); assert.ok(execution.observation.calls > 4300);
			assert.ok(execution.observation.rejections >= 67); assert.equal(execution.observation.records, 7);
			assert.equal(execution.observation.documentation, phpWasmCollectionDocumentation().stdout); assert.equal(execution.observation.word_bits, 32);
		...
		reports.push({ path, profile: "php-wasm", signatures
			, documentation: phpWasmCollectionDocumentation()
			, ...installed
	...
	const reportPath = resolve(process.env.LEAN_BRIDGE_PHP_WASM_COLLECTION_REPORT ?? "build/collections/php-wasm.json");
