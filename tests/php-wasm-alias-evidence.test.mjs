/**
 * Bind alias claims to installed wasm32 packages and separate ownership probes.
 *
 * @file
 */
...
import { phpWasmAliasConsumer, phpWasmAliasRequest, phpWasmAliasCatalog, phpWasmAliasContract } from "./helpers/php-wasm-alias-fixture.mjs";
import { phpWasmDriverHashes, phpWasmIsolationFlags } from "./helpers/type-corpus-php-wasm-evidence.mjs";
import { zendListFaultProvider, zendListFaultConsumer } from "./helpers/php-wasm-list-faults.mjs";
...
test("PHP-Wasm alias evidence covers installed public contracts and every loading arrangement", async () => {
	...
	for(const run of record.executions)
	{
		...
		assert.deepEqual(run.catalog.aliases, phpWasmAliasCatalog);
		for(const key of ["originalAliasChains", "originalApiSites", "originalRecordFields", "installedSourceDocumentation", "transparentTargetValues"]) assert.equal(run.catalog[key], true, key);
		assert.equal(run.contractSha256, sha256(canonicalJson(phpWasmAliasContract(nativeAliasReviewedIr()))));
		...
		{
			assert.equal(evidence.consumerSources[mode], sha256(phpWasmAliasConsumer(mode, run.path)));
			assert.equal(files[mode + ".php"].sha256, evidence.consumerSources[mode]);
		}
		for(const arrangement of ["embedded", "composer"])
		{
			assert.equal(evidence.requests[arrangement], sha256(phpWasmAliasRequest(arrangement)));
		...
		for(const execution of evidence.executions)
		{
			...
			assert.equal(result.checks, 12544); assert.equal(result.aliases, 27); assert.equal(result.word_bits, 32); assert.equal(result.php, "8.4.1");
			assert.deepEqual(result.primitives.map(item => item.name).sort(), Object.values(aliasPrimitives).sort());
			assert.ok(result.primitives.every(item => item.checks === 256));
			assert.equal(result.api, "/vendor/lean-bridge-aliases/wasm/src/Api.php");
	...
	const faults = record.faultProbe, ir = zendAliasFaultIr(), generated = generateCopiedPhpZendAdapter(ir);
	...
	for(const run of faults.executions) assert.deepEqual(run, { mode: run.mode, allocationFailures: 61, bailoutRecovery: 2, checks: 147, emptyPoisonPointer: 1, malformedOutputs: 10, partialInputs: 16, wordBits: 32 });
	const source = await readFile("tests/fixtures/alias-consumers/php-wasm.php", "utf8");
	assert.doesNotMatch(source, /Internal\\|FFI|CData|lean_ctor_|::check[0-9]|::from[0-9]/);
});

test("PHP-Wasm aliases promote only six copied cells and require installed CI checks", async () => {
	const { document, ...contracts } = await readTypeSurface();
	const cells = typeSurfaceCells(document, contracts).filter(cell => cell.stages.installedExecution.evidence.includes("php-wasm-aliases-installed"));
	assert.equal(cells.length, 6);
	for(const cell of cells)
	{
		assert.equal(cell.profile, "php-wasm"); assert.equal(cell.shape, "alias"); assert.ok(["parameter", "result", "field"].includes(cell.position));
		for(const stage of Object.values(cell.stages))
		{ assert.equal(stage.state, "passed"); assert.deepEqual(stage.evidence, ["php-wasm-aliases-installed"]); }
	}
	const workflow = await readFile(".github/workflows/consumer-matrix.yml", "utf8");
	assert.ok(workflow.includes("LEAN_BRIDGE_PHP_WASM_ALIAS_TEST=1 node --test --test-concurrency=1 tests/php-wasm-aliases.test.mjs tests/php-wasm-alias-contract.test.mjs tests/php-wasm-alias-zend.test.mjs"));
