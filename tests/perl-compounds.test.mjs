/**
 * Installed CPAN compounds on independently specified ordinary and reviewed paths.
 *
 * @file
 */
...
import { preparePerlCompounds } from "./helpers/perl-compound-install.mjs";

const enabled = process.env.LEAN_BRIDGE_PERL_COMPOUND_TEST === "1";
...
test("installed Perl compounds preserve copied values on both source paths", { skip: !enabled, timeout: 900_000 }, async t => {
	...
	{
		const author = await mkdtemp(join(tmpdir(), "lean-bridge-perl-compound-author-"));
		const consumer = await mkdtemp(join(tmpdir(), "lean-bridge-perl-compound-consumer-"));
		...
		t.diagnostic(`${path}: compiling Perl compounds for ${perls.length} ABIs`);
		...
		for(const [index, perl] of perls.entries())
		{
			...
			prepared.push(await preparePerlCompounds({ consumer: join(consumer, `abi-${index}`), handoff, packages: receipt.packages, perl, environment }));
		}
	...
	await saveLakeFile("build/compounds", "perl.json", canonicalJson({ schemaVersion: 1, reports }));
});
