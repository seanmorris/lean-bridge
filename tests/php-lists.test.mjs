/**
 * Installed native PHP lists under weak and strict callers on both paths.
 *
 * @file
 */
...
import { probePhpLists } from "./helpers/php-list-probe.mjs";

const enabled = process.env.LEAN_BRIDGE_PHP_LIST_TEST === "1";
...
test("installed native PHP lists preserve copied values on both source paths", { skip: !enabled, timeout: 1_800_000 }, async t => {
	const reports = [];
	...
	{
		const author = await mkdtemp(join(tmpdir(), "lean-bridge-php-list-author-"));
		const consumer = await mkdtemp(join(tmpdir(), "lean-bridge-php-list-consumer-"));
		...
		await saveLakeFile(projectRoot, "lean-bridge.exports.json", canonicalJson({ schemaVersion: 1
			, modules: ["Lists"]
			, targets: { "php-native": { name: "lean-bridge-lists/api", version: "1.0.0" } }
		...
		t.diagnostic(`${path}: compiling the 27-export native PHP list library`);
		...
		const installed = await installedPhpCorpus({ library: { phpModule: "LeanLists" }
			, consumer, handoff, pkg, environment
			, clean: copiedCleanEnvironment, sourcePath: path
			, fixture: { source: phpListConsumer, request: phpListRequest, removeHandoff: true } }).catch(error => { error.message += `: ${JSON.stringify(error.details)}`; throw error; });
		...
		const faults = await probePhpLists({ consumer, environment, installed, projection: compileCopiedPhpModel(model.bindingIr, { lists: true }) });
	...
	const reportPath = resolve(process.env.LEAN_BRIDGE_PHP_LIST_REPORT ?? "build/lists/php-native.json");
