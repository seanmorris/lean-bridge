...
test("List evidence covers npm, C/C++, Python, Rust, C#, JVM, Ruby, Perl and native PHP copied values without promoting other hosts or callables", () => {
	const profiles = ["node-javascript", "node-typescript", "browser-javascript", "browser-react", "browser-worker"];
	const cells = typeSurfaceCells(document, contracts).filter(cell => cell.shape === "list");
	assert.equal(cells.filter(cell => cell.stages.installedExecution.state === "passed").length, 90);
	...
	for(const cell of cells)
	{
		...
		const ruby = cell.profile === "ruby", perl = cell.profile === "perl", php = cell.profile === "php-native";
		if((npm || native || python || rust || dotnet || jvm || ruby || perl || php) && copied)
		{
			...
			for(const stage of Object.values(cell.stages))
			{ assert.equal(stage.state, "passed"); assert.deepEqual(stage.evidence, [php ? "php-native-lists-installed" : perl ? "perl-lists-installed" : ruby ? "ruby-lists-installed" : jvm ? "jvm-lists-installed" : dotnet ? "dotnet-lists-installed" : rust ? "rust-lists-installed" : python ? "python-lists-installed" : npm ? "npm-lists-installed" : "native-lists-installed"]); }
		} else
		{
...
for(const [profile, evidence] of [["php-native", "native-php-installed-copied"], ["php-wasm", "php-wasm-installed-copied"], ["rust", "native-rust-installed-copied"], ["dotnet", "native-dotnet-installed-copied"], ["java", "native-jvm-installed-copied"], ["kotlin", "native-jvm-installed-copied"], ["ruby", "native-ruby-installed-copied"], ["wit-wasi", "native-wit-installed-copied"], ["python", "native-python-installed-copied"]]) test(`ordinary ${profile} installed evidence stays within copied-value positions`, () => {
	...
	const lists = ["python", "rust", "dotnet", "java", "kotlin", "ruby", "php-native"].includes(profile) ? ["list"] : [];
