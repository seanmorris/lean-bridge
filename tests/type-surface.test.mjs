...
test("List evidence covers all copied profiles and independently accepted C/C++ callback payloads", () => {
	const profiles = ["node-javascript", "node-typescript", "browser-javascript", "browser-react", "browser-worker"];
	const cells = typeSurfaceCells(document, contracts).filter(cell => cell.shape === "list");
	assert.equal(cells.filter(cell => cell.stages.installedExecution.state === "passed").length, 110);
	...
	for(const cell of cells)
	{
		...
		} else if(native && cell.position.startsWith("callback-"))
		{
