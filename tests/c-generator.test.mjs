...
import {
	assembleCPackageLayout,
	CBindingGenerationError,
	compileCProjectionModel,
	generateCBindingPackage,
	renderCProjectionModel,
...
test("C projection, rendering, layout, and audit stages preserve generated bytes", () => {
	const model = compileCProjectionModel(alpha.bindingIr);
	const rendered = renderCProjectionModel(model);
	const files = assembleCPackageLayout(model, rendered);
	assert.deepEqual(files, generateCBindingPackage(alpha.bindingIr));
	assert.deepEqual(Object.keys(rendered.files), [
		"include/lean_alpha.h"
		, "internal/lean_alpha_runtime.h"
		, "src/lean_alpha.c"
		, "README.md"
	]);
	assert.equal(model.bindingIrSha256, JSON.parse(files["binding-manifest.json"]).bindingIrSha256);
	assert.doesNotThrow(() => auditCPackage(alpha.bindingIr, files));
});
