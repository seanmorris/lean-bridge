/**
 * Isolated PHP variant fault injection over a verified installed deployment.
 *
 * @file
 */
...
/**
 * Probe all named cases, then rerun untouched public consumers in new processes.
 *
 * @param options - Original installation and compiler-authenticated projection.
 * @param options.consumer - Scratch outside the installed deployment.
 * @param options.environment - Explicit PHP interpreter.
 * @param options.installed - Receipt-verified Composer installation.
 * @param options.projection - Private decoder indices, never public expectations.
 */
export const probePhpVariants = async ({ consumer, environment, installed, projection }) => {
	const deployment = join(consumer, "php-native/relocated");
	const source = await readFile("tests/fixtures/variant-consumers/php-faults.php", "utf8");
	...
	const run = await runCopied(environment.LEAN_BRIDGE_PHP, [...installed.php.runtimeOptions, join(consumer, "faults.php"), join(consumer, "probes.json")], deployment);
	assert.equal(run.stderr, "");
	const faults = JSON.parse(run.stdout);
	assert.ok(faults.failures > 200 && faults.checks > faults.failures * 4);
