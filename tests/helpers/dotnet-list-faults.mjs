...
/**
 * Exercise cleanup at each input/output conversion checkpoint and malformed buffers.
 *
 ...
 */
export const checkDotnetListFaults = async ({ consumer, environment, sources, projection }) => {
	...
	replace(/try \{ allocations.Add\(data\); \}/g, "ListProbe.Live++;\n        try { ListProbe.Check(); allocations.Add(data); }");
	replace(/catch \{ NativeMemory.Free\(\(void\*\)data\); throw; \}/g, "catch { ListProbe.Live--; NativeMemory.Free((void*)data); throw; }");
	replace(/foreach \(var data in allocations\) NativeMemory.Free\(\(void\*\)data\);/g, "foreach (var data in allocations) { ListProbe.Live--; NativeMemory.Free((void*)data); }");
	replace(/( {12}var status = Native.Call\d+)/g, "            ListProbe.Calls++;\n$1", 27);
	replace(/finally \{ (Native.Clear\d+\(ref output\);) \}/g, "finally { ListProbe.Clears++; $1 }", 27);
	runtime += `\ninternal static class ListProbe {
	...
	const program = await readFile("tests/fixtures/list-consumers/dotnet-faults.cs", "utf8");
	...
	const result = await runCopied(environment.LEAN_BRIDGE_DOTNET, ["out/Probe.dll", String(index("reverse_uint32")), String(index("mix"))], root, env);
	assert.equal(result.stderr, ""); assert.match(result.stdout, /^list-dotnet-faults:\d+:9\n$/);
	const checks = Number(result.stdout.split(":")[1]); assert.ok(checks > 50);
	return { checks, layoutChecks: 9, partialInputChecks: 16, replacements
