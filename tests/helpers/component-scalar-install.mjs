...
const compile = async (fixture, projectRoot, root, local = false) => {
	...
	if(fixture.requiredRuntimeSymbol)
	{
		...
		await assert.rejects(() => buildComponentNpmPackages({ bundleRoot, runtimeRoot: oldRuntime, outputRoot: join(root, "rejected-npm") }), /cannot resolve component import|lacks the component (copied|record|compound|nominal|recursive) ABI/);
	}
	return release;
};
