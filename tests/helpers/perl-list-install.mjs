 ...
 */
export const preparePerlLists = async ({ consumer, handoff, packages, perl, environment }) => {
	...
	const sources = Object.fromEntries(await Promise.all(["perl.pl", "perl-faults.pl", "perl-probe.h", "perl-probe-build.pl"].map(async name => [name, await readFile(`tests/fixtures/list-consumers/${name}`, "utf8")])));
...
MODULE = LeanBridge::Lists    PACKAGE = LeanBridge::Lists
...
`;
	...
	const apiPaths = Object.keys(installedFiles).filter(path => path.endsWith("/LeanBridge/Lists.pm"));
