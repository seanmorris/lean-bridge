/**
 * Independent Component Model shapes for copied compound acceptance.
 *
 * @file
 */
 ...
 */
export const validateWitCompoundSignatures = document => {
	const value = ref => {
	...
	const signatures = compoundSignatures.map(fn => ({ name: fn.name.split(".").at(-1).replaceAll("_", "-")
...
export const witCompoundConsumer = async () => {
...
`;
	const oracles = primitive.slice(start, end).replaceAll("assert(", "CHECK(").replace("  default: abort();\n  }\n}\n", branches);
	assert.ok(oracles.includes("case WASMTIME_COMPONENT_OPTION:"));
	return (await readFile("tests/fixtures/compound-consumers/wit-wasi.c", "utf8")).replace("/* COPIED_ORACLES */", oracles);
};
