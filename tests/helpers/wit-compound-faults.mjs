...
export const witCompoundFaultIr = () => corpusReviewedIr({ id: "probe" }, [{ name: "Probe.echo"
	, parameters: [{ array: { tuple: [{ option: "nat" }, { result: ["string", "bytes"] }] } }]
	, result: { array: { tuple: [{ option: "nat" }, { result: ["string", "bytes"] }] } } }]);
 ...
 */
export const witCompoundFaultSource = model => {
...
#include <stdio.h>
#include <string.h>
static size_t attempts, fail_at, live, checks, failures, budget_failures;
...
int main(void) {
  ...
  printf("{\\"checks\\":%zu,\\"scratchFailures\\":%zu,\\"budgetFailures\\":%zu,\\"malformedOutputs\\":8,\\"inactivePayloads\\":3,\\"liveAllocations\\":%zu}\\n", checks, failures, budget_failures, live);
}
`;
};
