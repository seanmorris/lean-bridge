 ...
 */
export const installRubyCompounds = async ({ consumer, handoff, packages, environment, projection }) => {
	...
	const locate = ["-e", 'print Gem::Specification.find_by_name("compounds-api", "1.0.0").full_gem_path'];
	...
	assert.equal(receipt.name, "compounds-api"); assert.equal(receipt.version, "1.0.0");
	await verifyNativeFiles(installed, receipt.files);
	const sources = Object.fromEntries(await Promise.all(["ruby", "ruby-faults"].map(async name => [name, await readFile(`tests/fixtures/compound-consumers/${name}.rb`, "utf8")])));
	...
	const execute = async () => {
		...
		assert.equal(observation.api, join(deployed, "lib/lean_bridge/compounds.rb"));
		assert.deepEqual(observation.native_libraries, expectedLibraries);
		return observation;
	};
	const first = await execute();
	const indices = ["option_string", "result_string"].map(name => projection.surface.copy(projection.surface.functions.find(fn => fn.field === name).declaration.result.type).index);
	...
	assert.ok(faults.checks > 100); assert.equal(faults.partial_inputs, 16); assert.equal(faults.flags, 9);
