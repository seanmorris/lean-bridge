 ...
 * @param options.projection - Native type indices for flag checks, not semantic expectations.
 */
export const checkJvmCompoundFaults = async ({ consumer, environment, sources, jar, projection }) => {
	const root = join(consumer, "compound-probe"), prefix = "src/main/java/org/leanbridge/compounds/", replacements = [];
	const files = { ...sources };
	...
	replace(runtime, /Arena arena = Arena\.ofConfined\(\)/g, "Arena arena = CompoundProbe.arena()", 64);
	for(const file of [runtime, scope]) replace(file, /\b(scope\.arena|arena)\.allocate\(/g, "CompoundProbe.allocate($1, ");
	replace(runtime, /( {16}int status = \(int\)CALL\d+)/g, "                CompoundProbe.calls++;\n$1", 64);
	replace(runtime, /finally \{ (CLEAR\d+\.invokeExact\(output\);) \}/g, "finally { CompoundProbe.clears++; $1 }", 60);
	const probe = `package org.leanbridge.compounds;
import java.lang.foreign.*;
final class CompoundProbe {
    private CompoundProbe() { }
...
`;
	const program = await readFile("tests/fixtures/compound-consumers/jvm-faults.java", "utf8");
	files[`${prefix}CompoundProbe.java`] = probe; files[`${prefix}Faults.java`] = program;
	...
	const result = await runCopied(environment.LEAN_BRIDGE_JAVA, ["--enable-native-access=ALL-UNNAMED", `-Djava.io.tmpdir=${temp}`, "-cp", `classes:${jar}`, "org.leanbridge.compounds.Faults", String(index("option_string")), String(index("result_string"))], root, copiedCleanEnvironment);
	assert.equal(result.stderr, ""); assert.match(result.stdout, /^compound-jvm-faults:\d+:9\n$/);
