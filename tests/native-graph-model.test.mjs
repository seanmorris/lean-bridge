...
test("ordinary and reviewed native graph components execute the independent C stress caller", {
	...
	for(const reviewed of [false, true])
	{
		...
		try
		{
			const result = await checkNativeRecursiveTransport(directory, { component: true, reviewed });
			...
			t.diagnostic(`${reviewed ? "reviewed" : "ordinary"} component: ${result.checks} checks, binary ${result.binarySha256}`);
		}
		finally
