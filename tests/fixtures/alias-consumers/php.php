...
// Independent installed consumer. Calls use only the public package API.
...
function rejected(string $kind, callable $call): Throwable {
    try { $call(); } catch (Throwable $error) {
        ...
        check(LeanAliases\increment(41) === 42, 'recovery');
        return $error;
    }
...
check($root === realpath(__DIR__ . '/vendor/lean-bridge-aliases/api'));
...
$values = [null, true, 255, 65535, 4294967295, $big('18446744073709551615'), -128, -32768, -2147483648,
    PHP_INT_MIN, $huge, $negative, 1.5, -2.25, "A\0🌱", Bytes::fromString("\0\xff\1"), '🌱', $big('4294967295'), -2147483648];
...
same(LeanAliases\make(), 41); same(LeanAliases\increment(LeanAliases\make()), 42);
same(LeanAliases\increment(4294967295), 0); same(LeanAliases\label(), 'alias🌱');
foreach ([null, new Some(null), new Some(new Some(null))] as $maybe) same(LeanAliases\echo_maybe($maybe), $maybe);
foreach ([new Ok([7, Bytes::fromString("\0\xff")]), new Err("bad\0🌱")] as $outcome) same(LeanAliases\echo_outcome($outcome), $outcome);
for ($i = 0; $i < 32; ++$i) {
    $packet = new Packet($i, "text\0🌱", [[1, 2, 3], [], [$i]], new Some(new Some(null)), new Ok([7, Bytes::fromString("\0\xff")]));
    $out = LeanAliases\change_packet($packet);
    same($out, new Packet($i + 1, $packet->text, $packet->rows, $packet->maybe, $packet->outcome));
...
same(LeanAliases\reverse_rows([[1, 2, 3], [], [4]]), [[3, 2, 1], [], [4]]);
$row = [1, 2]; $rows = [&$row, &$row]; $out = LeanAliases\reverse_rows($rows);
$row[0] = 99; same($out, [[2, 1], [2, 1]]); $out[0][0] = 7; same($out[1], [2, 1]);
same(LeanAliases\duplicate(Bytes::fromString("\0\xff")), new Ok([7, Bytes::fromString("\0\xff\0\xff")]));

$invalid = [
    ['unit', false, TypeError::class], ['bool', 1, TypeError::class], ['uint8', -1, ValueError::class], ['uint8', 256, ValueError::class],
    ['uint16', 65536, ValueError::class], ['uint32', 4294967296, ValueError::class], ['uint32', true, TypeError::class],
...
foreach ([null, false, 42, '1', (object) [], new ArrayObject([1, 2]), [1 => []], ['x' => []], [0 => [], 2 => []], [[1], ['2']]] as $bad)
...
rejected(ValueError::class, fn() => LeanAliases\reverse_rows([array_fill(0, 400000, 0)]));
...
echo json_encode(['checks' => $checks, 'primitives' => $primitives, 'php' => PHP_VERSION, 'word_bits' => PHP_INT_SIZE * 8,
    'aliases' => count($manifest['aliases']), 'api' => $api, 'native_libraries' => $libraries], JSON_THROW_ON_ERROR) . "\n";
