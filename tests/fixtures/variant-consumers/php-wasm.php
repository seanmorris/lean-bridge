...
// Independent public consumer of installed wasm32 npm and Composer archives.
...
check($root === '/vendor/lean-bridge-variants/wasm');
...
$values = ['unit' => null, 'bool' => true, 'u8' => 255, 'u16' => 65535, 'u32' => $big('4294967295'), 'u64' => $big('18446744073709551615'),
    'i8' => -128, 'i16' => -32768, 'i32' => -2147483647 - 1, 'i64' => $big('-9223372036854775808'),
    'natural' => $big(2)->power(5120)->plus(19), 'integer' => $big(2)->power(5120)->plus(31)->negated(),
    'f32' => 1.5, 'f64' => -2.25, 'text' => "A\0🌱", 'bytes' => Bytes::fromString("\0\xff\1"), 'char' => '🌱', 'word' => $big('4294967295'), 'signedWord' => -2147483647 - 1];
...
$signals = [new SignalIdle(), new SignalStopped(), new SignalData(count: $big(42), label: "A\0🌱"), new SignalMarker(value: null)];
for ($round = 0; $round < 128; ++$round) {
    ...
    same(call('next', $signals[2]), new SignalData($big(43), "A\0🌱!")); same(call('next', $signals[3]), new SignalData($big(42), 'ready'));
    foreach ([7, 13, 48, 29] as $index => $code) same(call('code', $signals[$index]), $big($code));
    foreach ([$round % 3, 3, 7] as $code) same(call('make', $big($code)), $code === 0 ? new SignalIdle() : new SignalData($big($code), 'made'));
    ...
    foreach ([new AnonymousNumber(arg0: $big(13)), new AnonymousPair(arg0: $big(17), arg1: "A\0🌱"), new AnonymousCollision(arg1: $big(19), arg1_: "A\0🌱")] as $value) same(call('echo_anonymous', $value), $value);
    same(call('echo_one', new OneOnly($big($round))), new OneOnly($big($round + 1)));
...
foreach (['0000000000000000', '8000000000000000', '0000000000000001', '000fffffffffffff', '0010000000000000', '3ff0000000000000', '7fefffffffffffff', '7ff0000000000000', 'fff0000000000000', '7ff8000000000000'] as $hex) {
    $special = unpack('E', hex2bin($hex))[1]; $input = new ScalarsAll(...array_replace($values, ['f32' => $special, 'f64' => $special, 'word' => $big('4294967295'), 'signedWord' => PHP_INT_MIN]));
...
foreach ([['unit', 1, TypeError::class], ['bool', 1, TypeError::class], ['u8', -1, ValueError::class], ['u8', 256, ValueError::class],
    ['u16', 65536, ValueError::class], ['u32', $big('4294967296'), ValueError::class], ['u32', 1, TypeError::class], ['u32', true, TypeError::class],
    ['u64', $big(-1), ValueError::class], ['u64', $big('18446744073709551616'), ValueError::class],
    ['i8', -129, ValueError::class], ['i16', 32768, ValueError::class], ['i32', -2147483649.0, TypeError::class],
    ['i64', '1', TypeError::class], ['i64', 1, TypeError::class], ['i64', $big('-9223372036854775809'), ValueError::class], ['i64', $big('9223372036854775808'), ValueError::class], ['signedWord', true, TypeError::class], ['word', 1, TypeError::class], ['word', $big('4294967296'), ValueError::class],
...
foreach (['count' => $big(-1), 'label' => "\xff"] as $field => $bad) {
    $object = (new ReflectionClass(SignalData::class))->newInstanceWithoutConstructor();
    foreach (array_replace(['count' => $big(42), 'label' => 'ok'], [$field => $bad]) as $name => $value) (new ReflectionProperty(SignalData::class, $name))->setValue($object, $value);
    rejected(ValueError::class, fn() => call('echo_signal', $object));
}
rejected(Error::class, fn() => new Signal()); rejected(ArgumentCountError::class, fn() => new SignalData($big(1)));
rejected(ArgumentCountError::class, fn() => new SignalIdle(1)); rejected(Error::class, fn() => new SignalData(count: $big(1), label: 'ok', extra: 2));
...
rejected(ValueError::class, fn() => new SignalData($big(1), str_repeat('x', 16 * 1024 * 1024)));
...
echo json_encode(['checks' => $checks, 'calls' => $calls, 'rejected' => $rejected, 'primitives' => $primitives,
    ...
    'word_bits' => PHP_INT_SIZE * 8, 'api' => $api], JSON_THROW_ON_ERROR) . "\n";
