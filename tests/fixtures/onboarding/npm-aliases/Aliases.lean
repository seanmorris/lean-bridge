...
def scalars (value : ScalarsView) : ScalarsView := value
...
def packet (value : PacketView) : PacketView := { value with count := value.count + 1 }
def packets (value : Packets) : Packets := value.reverse
def rows (value : Rows) : Rows := value.map List.reverse
def maybe (value : Maybe) : Maybe := value
def outcome (value : Outcome) : Outcome := value
def mode (value : ModeView) : ModeView := match value with
  | .first => .second 42
