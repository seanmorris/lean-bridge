...
def echo_mode (value : Mode) : Mode := value
def echo_nested (value : Nested) : Nested := value
def signals (value : Array (List Signal)) : Array (List Signal) := value.map List.reverse
def echo_scalars (value : Scalars) : Scalars := value
def echo_anonymous (value : Anonymous) : Anonymous := value
def echo_one (value : One) : One := match value with | .only n => .only (n + 1)
