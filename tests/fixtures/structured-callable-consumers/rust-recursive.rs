...
fn tree_value(seed: u32) -> api::Tree {
    match seed % 3 {
        0 => api::Tree::Leaf { value: (BigUint::from(1u8) << (257 + seed as usize)) + seed },
        1 => api::Tree::Branch { children: vec![] },
        _ => api::Tree::Branch { children: vec![tree_value(0), tree_value(1)] },
    }
}
fn main() -> Result<(), Error> {
    ...
    for seed in 0..12 {
        ...
        exercise!(call_recursive, twice_recursive, make_recursive, tree_value, seed);
    }
    ...
    check(live() == baseline);
    for depth in 0..64 {
        let mut tree = tree_value(0);
        for _ in 0..depth { tree = api::Tree::Branch { children: vec![tree] }; }
        same(&api::call_recursive(&tree, Ok)?, &tree);
        let held = api::make_recursive(&tree)?;
        same(&held.call(true, &tree_value(1))?, &tree);
    }
    let mut deep = tree_value(0);
    for _ in 0..64 { deep = api::Tree::Branch { children: vec![deep] }; }
    check(matches!(api::call_recursive(&deep, Ok), Err(Error::Limit)));
    check(matches!(api::call_recursive(&tree_value(0), |_| Ok(deep.clone())), Err(Error::Limit)));
    same(&api::call_recursive(&tree_value(0), |_| Err(Error::InvalidNative)), &Err(Error::InvalidNative));
    same(&api::call_recursive(&tree_value(0), Ok)?, &tree_value(0));
    {
        let value = record_value(1);
        let expected = format!("{}<none>{}", value.text, value.text);
        same(&api::call_nested_alias(&value, Ok)?, &expected);
        same(&api::call_nested_plain(&value, Ok)?, &expected);
        let alias = api::make_nested_alias(&value)?;
        let plain = api::make_nested_plain(&value)?;
        let rows = vec![Some(value.clone()), None, Some(value.clone())];
        same(&alias.call(&[])?, &rows);
        same(&plain.call(&[])?, &rows);
    }
    {
        let mut captures = Vec::with_capacity(4096);
        for _ in 0..4096 { captures.push(api::make_recursive(&tree_value(0))?); }
        check(matches!(api::make_recursive(&tree_value(0)), Err(Error::Allocation)));
        captures.pop().unwrap().close()?;
        let replacement = api::make_recursive(&tree_value(0))?;
        same(&replacement.call(true, &tree_value(1))?, &tree_value(0));
    }
    check(live() == baseline);
    println!("recursive-rust-ok:{}", CHECKS.load(Ordering::Relaxed));
    Ok(())
}
