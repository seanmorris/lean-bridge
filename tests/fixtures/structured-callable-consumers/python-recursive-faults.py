"""Failure injection into the installed adapter in memory, never its files."""
import ctypes
import gc
import json
import re
...
def exercise(function, expected, fail_at=None, exception=MemoryError):
    ...
    replacements = {}
    checkpoints = 0
    marker = exception('recursive checkpoint failure')
    allocate, value, close = native._GraphScope.allocate, native._GraphScope.value, native._GraphScope.close
    failed = False
    ...
    def track_close(scope):
        global closes
        close(scope)
        check(not scope.owners and not scope.active)
    ...
    clear = native._graph_clear
    def track_clear(output):
        global clears
        clear(output)
        if output is not None and hasattr(output, 'owner'):
            check(ctypes.string_at(ctypes.addressof(output), ctypes.sizeof(output)) == bytes(ctypes.sizeof(output)))
            clears += 1

    for name, original in list(vars(native).items()):
        if re.fullmatch(r'_graph_(?:input|output)\d+|_graph_own|_callback\d+', name):
            replacements[name] = original
            setattr(native, name, checkpoint(original))
    original_checkpoint = native._graph_checkpoint
    native._graph_checkpoint = tick
    native._graph_clear = track_clear
    native._GraphScope.allocate = track_memory(allocate)
    native._GraphScope.value = track_memory(value)
    native._GraphScope.close = track_close
    try:
        ...
        native._GraphScope.allocate = allocate
        native._GraphScope.value = value
        native._GraphScope.close = close
        native._graph_checkpoint = original_checkpoint
...
for shape in values['SHAPES'] + ('recursive',):
    value = (api.TreeBranch([api.TreeLeaf(1 << 200), api.TreeBranch([])])
             if shape == 'recursive' else values['payload'](shape, 2 if shape == 'option' else 1))
    other = (api.TreeBranch([api.TreeLeaf(2), api.TreeBranch([api.TreeLeaf(1 << 300)])])
             if shape == 'recursive' else values['payload'](shape, 6 if shape == 'variant' else 2))
    expected = values['owned'](other)
...
for name, value_type in list(vars(native).items()):
    if not re.fullmatch(r'_Raw\d+', name):
        continue
    decode = getattr(native, '_graph_output' + name[4:])
...
print(json.dumps({'checks': checks, 'faults': faults, 'clears': clears, 'closes': closes, 'identities': live(),
                  'malformed': malformed, 'shapes': reports}, sort_keys=True))
