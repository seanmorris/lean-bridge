...
function rejected(string $kind, callable $call): Throwable {
    try { $call(); } catch (Throwable $error) {
        ...
        check(array_map('strval', LeanLists\reverse_uint32(words([1, 2, 3]))) === ['3', '2', '1'], 'recovery');
        return $error;
    }
...
function words(array $values): array { return array_map(fn($value) => BigInteger::of((string) $value), $values); }
...
$cases = [
    ...
    'int32' => [PHP_INT_MIN, -1, 0, PHP_INT_MAX], 'int64' => array_map($big, ['-9223372036854775808', '-9007199254740993', '-1', '0', '9223372036854775807']),
...
$invalid = [
    ...
    ['uint16', 65536, ValueError::class], ['uint32', $big('4294967296'), ValueError::class], ['uint32', 1, TypeError::class],
    ...
    ['int64', $big('9223372036854775808'), ValueError::class], ['int64', $big('-9223372036854775809'), ValueError::class],
    ['usize', $big('4294967296'), ValueError::class], ['isize', 2147483648.0, TypeError::class],
...
same(LeanLists\mix([words([1, 2, 3]), [], words([4])]), [words([4]), [], words([3, 2, 1])]);
for ($i = 0; $i < 20; ++$i) {
    $input = new Packet([words([1, 2, 3]), [], words([$i])], [null, new Some(new Ok([$huge, null])), new Some(new Err("oops\0"))],
        [Bytes::fromString("\0\xff"), Bytes::fromString('')], [[[true, '🌿'], [false, "\0"]], []]);
    $output = LeanLists\transform($input);
    same($output, new Packet([words([$i]), [], words([3, 2, 1])], [new Some(new Err("oops\0!")), new Some(new Ok([$huge->plus(1), null])), null],
    ...
    $sequences = $output->sequences; $sequences[2][0] = 99; same($input->sequences[0], words([1, 2, 3]));
...
same(LeanLists\swap(new Ok([[$huge, $big(42)], words([1, 2, 3])])), new Err([[$big(42), $huge], words([3, 2, 1])]));
...
$shared = words([1, 2]); $input = [&$shared, &$shared]; $copied = LeanLists\mix($input);
$shared[0] = 99; same($copied, [words([2, 1]), words([2, 1])]); $copied[0][0] = 7; same($copied[1], words([2, 1]));
...
$deep = $big(42); for ($i = 0; $i < 25; ++$i) $deep = [$deep];
...
rejected(TypeError::class, fn() => LeanLists\mix([words([1]), new ArrayObject(words([2]))]));
foreach ([[], [[]], [[], [], []], ['a' => [], 'b' => []]] as $bad)
...
rejected(ValueError::class, fn() => LeanLists\reverse_uint32(array_fill(0, 2097153, $big(0))));
for ($i = 0; $i < 3; ++$i) rejected(LeanBridgeError::class, fn() => LeanLists\duplicate(Bytes::fromString(str_repeat('x', 6 * 1024 * 1024))));
rejected(LeanBridgeError::class, fn() => LeanLists\generate($big(4194305)));
rejected(ValueError::class, fn() => LeanLists\generate($big(400000)));
same(LeanLists\generate($big(0)), []); same(LeanLists\generate($big(1)), words([7]));
same(LeanLists\generate($big(30000)), array_fill(0, 30000, $big(7)));
gc_collect_cycles(); same(LeanLists\reverse_uint32(words([1, 2, 3])), words([3, 2, 1]));
$api = (new ReflectionFunction(LeanLists\reverse_uint32(...)))->getFileName();
check($api === '/vendor/lean-bridge-lists/wasm/src/Api.php');
echo json_encode(['checks' => $checks, 'primitives' => $primitives, 'php' => PHP_VERSION, 'word_bits' => PHP_INT_SIZE * 8,
    'api' => $api], JSON_THROW_ON_ERROR) . "\n";
