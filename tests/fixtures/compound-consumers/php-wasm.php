...
function rejected(string $kind, callable $call): Throwable {
    try { $call(); } catch (Throwable $error) {
        ...
        check((string) LeanCompounds\classify(new Some(new Some(null))) === '2', 'recovery');
        return $error;
    }
...
for ($i = 0; $i < 30; ++$i) {
    same($state, $states[$i % 3]); check((string) LeanCompounds\classify($state) === (string) ($i % 3));
    $state = LeanCompounds\next($state);
}
same(LeanCompounds\make(), new Some(new Ok([$big('18446744073709551615'), null])));
same(LeanCompounds\flip(new Ok([$big(42), new Some(null)])), new Err([$big(42), new Some(null)]));
same(LeanCompounds\flip(new Ok([$big(0), null])), new Err([$big(0), null]));
...
foreach ([null, new Some(new Ok([$huge, null])), new Some(new Err("oops\0"))] as $choice) {
    foreach ([new Ok(null), new Ok(new Some(new Ok([$big(42), null]))), new Ok(new Some(new Err('bad'))), new Err(null), new Err(new Some($huge))] as $inside) {
        $rows = [null, new Some(new Ok(["row\0🌿", $big('18446744073709551615')])), new Some(new Err([Bytes::fromString("\0\xff"), $negative]))];
        $input = new Packet($choice, [[$big(4), "a\0"], [true, '🌿']], $rows, $inside);
        ...
        same($output, new Packet($expected, [[$big(5), "a\0!"], [false, '🌿']], array_reverse($rows), $inside));
...
for ($depth = 0; $depth <= 24; ++$depth) {
    $value = $depth === 24 ? new Ok([$big(42), null]) : null;
...
$api = (new ReflectionFunction(LeanCompounds\classify(...)))->getFileName();
check($api === '/vendor/lean-bridge-compounds/wasm/src/Api.php');
