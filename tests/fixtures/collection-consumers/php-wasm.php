...
function rejected(string $kind, callable $call): Throwable {
    ...
    try { $call(); } catch (Throwable $error) {
        ...
        same(callPublic('record_make'), new Pair(BigInteger::of(42), "\u{feff}🌱\0")); return $error;
    }
...
check(PHP_INT_SIZE === 4 && !PHP_ZTS); check(count($request['signatures']) === 35);
$recordTypes = [];
...
$arguments = [null, true, 255, 65535, $big('4294967295'), $max, -128, -32768, PHP_INT_MIN, $big('-9223372036854775808'),
    $big(2)->power(200), $big(2)->power(200)->negated(), -0.0, 3.25, "🌱\0", Bytes::fromString("\xff\0\1"), '🌱', $big('4294967295'), PHP_INT_MIN];
...
$changed = [null, false, 0, 0, $big(0), $big(0), 0, 0, 0, $big(0), $big(0), $big(0), 0.0, 0.0, '', Bytes::fromString(''), "\0", $big(0), 0];
...
$cases = [
    'unit' => [null], 'bool' => [false, true], 'uint8' => [0, 1, 255], 'uint16' => [0, 1, 65535], 'uint32' => array_map($big, ['0', '1', '2147483648', '4294967295']),
    'uint64' => array_map($big, ['0', '1', '4294967296', '9007199254740993', '18446744073709551615']),
    'int8' => [-128, -1, 0, 127], 'int16' => [-32768, -1, 0, 32767], 'int32' => [PHP_INT_MIN, -1, 0, PHP_INT_MAX],
    'int64' => array_map($big, ['-9223372036854775808', '-9007199254740993', '-1', '0', '9223372036854775807']), 'nat' => [$big(0), $big(1), $huge], 'int' => [$huge->negated(), $big(0), $huge],
...
$cases['usize'] = $cases['uint32']; $cases['isize'] = $cases['int32']; $primitives = [];
...
for ($i = 0; $i < 128; ++$i) {
    $input = new Packet('packet', [[$scalar, $scalar], [], [$scalar]], new Empty_(), new Single($max), new Count($huge), new Pair($big('4294967295'), 'pair'), new Reversed('reverse', $big('4294967295')));
    $expected = new Packet('packet!', [[$scalar], [], [$scalar, $scalar]], new Empty_(), new Single($big(0)), new Count($huge->plus(7)), new Pair($big(0), 'pairp'), new Reversed('reverser', $big(1)));
    ...
    same(callPublic('record_make'), new Pair(BigInteger::of(42), "\u{feff}🌱\0"));
...
for ($depth = 0; $depth <= 24; ++$depth) {
    $value = $depth === 24 ? $big(42) : []; for ($i = 0; $i < $depth; ++$i) $value = [$value];
...
$invalid = [
    ...
    ['uint16', 65536, ValueError::class], ['uint32', $big('4294967296'), ValueError::class], ['uint32', 1, TypeError::class], ['uint64', $big(-1), ValueError::class],
    ['uint64', $big('18446744073709551616'), ValueError::class], ['int8', -129, ValueError::class], ['int16', 32768, ValueError::class],
    ['int32', -2147483649.0, TypeError::class], ['isize', 2147483648.0, TypeError::class], ['usize', $big('4294967296'), ValueError::class],
    ['int64', $big('9223372036854775808'), ValueError::class], ['int64', $big('-9223372036854775809'), ValueError::class], ['int64', '1', TypeError::class], ['isize', true, TypeError::class], ['usize', 1, TypeError::class],
...
rejected(ArgumentCountError::class, fn() => new Empty_(1)); rejected(ArgumentCountError::class, fn() => new Pair($big(1), '', 3));
...
$deep = $big(42); for ($i = 0; $i < 25; ++$i) $deep = [$deep]; rejected(TypeError::class, fn() => callPublic('deep', $deep));
...
$row = [$big(1), $big(2)]; $value = [&$row, &$row]; $copied = callPublic('array_reverse_uint32', $value);
$row[0] = 99; same($copied, [[$big(2), $big(1)], [$big(2), $big(1)]]); $copied[0][0] = 7; same($copied[1], [$big(2), $big(1)]);
rejected(ValueError::class, fn() => callPublic('array_reverse_string', [[str_repeat('x', 17 * 1024 * 1024)]]));
rejected(ValueError::class, fn() => callPublic('array_reverse_uint32', [array_fill(0, 2097153, $big(0))]));
...
rejected(ValueError::class, fn() => callPublic('generate', $big(400000)));
same(callPublic('generate', $big(0)), []); same(callPublic('generate', $big(30000)), array_fill(0, 30000, null));
/* DOCUMENTATION */
gc_collect_cycles(); same(callPublic('record_make'), new Pair(BigInteger::of(42), "\u{feff}🌱\0"));
$api = (new ReflectionFunction(LeanCollections\record_make(...)))->getFileName();
check($api === '/vendor/lean-bridge-collections/wasm/src/Api.php');
echo json_encode(['checks' => $checks, 'calls' => $calls, 'rejections' => $rejections, 'primitives' => $primitives, 'records' => count($recordTypes),
    'php' => PHP_VERSION, 'word_bits' => PHP_INT_SIZE * 8, 'api' => $api, 'documentation' => $documentationOutput], JSON_THROW_ON_ERROR) . "\n";
