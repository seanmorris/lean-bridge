...
foreach ([0 => false, 1 => 1, 2 => '255', 3 => -1, 4 => 4294967296, 5 => 1, 6 => -129, 7 => -32769,
    8 => -2147483649, 9 => 1.0, 10 => Big::of(-1), 11 => '1', 12 => 1, 13 => '1.5', 14 => "\xff", 15 => '',
    16 => 'ab', 17 => Big::of('18446744073709551616'), 18 => '0'] as $index => $value) {
    reject(fn() => new Scalars(...array_replace($fields, [$index => $value])), in_array($index, [3, 4, 6, 7, 8, 10, 14, 16, 17], true) ? ValueError::class : TypeError::class);
}
check(!preg_match('~/liblean(?:shared|_bridge_native)\.so(?: \(deleted\))?$~m', file_get_contents('/proc/self/maps')));
if (getenv('GRAPH_COLD') === '1') {
    check(!extension_loaded('FFI'));
    echo json_encode(['checks' => $checks, 'rejections' => $rejections, 'exports' => count($signatures), 'ffiLoaded' => false], JSON_THROW_ON_ERROR) . "\n";
    exit;
}
check(inspect($scalars) === true);
check(word_max(Big::of('18446744073709551615')) === true);
...
$left = new LeftTreeNext(new RightTreeMany([new LeftTreeLeaf(9)])); check(left($left)->equals($left));
...
check(grow(new SpineLeaf(7))->equals(new SpineNext(new SpineLeaf(7))));
$wide = range(0, 254); $wide[] = new WideLeaf(17); $wide = new WideNext(...$wide); check(wide($wide)->equals($wide));
...
check(getenv('PATH') === '/unavailable'); check(getenv('LEAN_BRIDGE_NATIVE_ROOT') === '/unavailable/runtime');
$root = __DIR__ . '/vendor/lean-bridge/recursive';
$receipt = json_decode(file_get_contents($root . '/lean-bridge/package-receipt.json'), true, 512, JSON_THROW_ON_ERROR);
$maps = file_get_contents('/proc/self/maps'); $libraries = [];
foreach ($receipt['files'] as $path => $identity) if (str_starts_with($path, 'native/linux-x64/')) {
    $real = realpath($root . '/' . $path); check(str_contains($maps, ' ' . $real . "\n"));
    check(hash_file('sha256', $real) === $identity['sha256']); $libraries[basename($path)] = $identity['sha256'];
}
check(count($libraries) === 4); ksort($libraries);
$manifest = json_decode(file_get_contents($root . '/binding-manifest.json'), true, 512, JSON_THROW_ON_ERROR);
check(array_column($manifest['aliases'], 'name') === ['Forest', 'TreeAlias']);
...
echo json_encode(['checks' => $checks, 'rejections' => $rejections, 'exports' => count($signatures),
    'hostVersion' => PHP_VERSION, 'actualPhpBits' => PHP_INT_SIZE * 8, 'nativeLibraries' => $libraries,
