/**
 * Alias-wrapped wasm32 Zend ownership probes, separate from compiled Lean evidence.
 *
 * @file
 */
...
const enabled = process.env.LEAN_BRIDGE_PHP_WASM_ALIAS_TEST === "1";
test("wasm32 Zend aliases release nested scratch and output owners on failures and bailouts", { skip: !enabled, timeout: 240000 }, async t => {
	const root = await mkdtemp(join(tmpdir(), "lean-bridge-zend-alias-fault-"));
	...
	assert.ok(manifest.aliases.length > 15);
	assert.equal(manifest.aliases.length, ir.types.filter(type => type.kind === "alias").length);
	...
	await saveLakeFile(root, "provider.c", zendListFaultProvider(ir));
	...
	const consumer = zendListFaultConsumer(manifest);
...
for(const mode of [7,8]) {
  stdout=''; stderr='';
  await php.run("<?php LeanListProbe\\\\configure("+mode+"); LeanListProbe\\\\strings(['first', 'second']);");
  const status=await php.run("<?php LeanListProbe\\\\configure(0); echo LeanListProbe\\\\live_allocations(), ':', LeanListProbe\\\\strings(['recovered'])[0];");
...
`);
	...
	for(const mode of ["0", "1"])
	{
		...
		assert.equal(result.malformedOutputs, 10); assert.equal(result.partialInputs, 16); assert.equal(result.emptyPoisonPointer, 1); assert.equal(result.bailoutRecovery, 2);
		executions.push({ mode: mode === "1" ? "strict" : "weak", ...result });
	}
	const report = resolve("build/aliases/php-wasm-zend-faults.json");
	await saveLakeFile(dirname(report), report.split("/").at(-1), canonicalJson({ schemaVersion: 1
		, provider: "synthetic-not-Lean", aliases: manifest.aliases
		...
		, providerSha256: sha256(zendListFaultProvider(ir))
