...
const enabled = process.env.LEAN_BRIDGE_PHP_WASM_LIST_TEST === "1";
test("wasm32 Zend lists release scratch and output owners on failures and bailouts", { skip: !enabled, timeout: 240000 }, async t => {
	const root = await mkdtemp(join(tmpdir(), "lean-bridge-zend-list-fault-"));
	t.after(() => rm(root, { recursive: true, force: true }));
	const ir = zendListFaultIr(), files = generateCopiedPhpZendAdapter(ir);
...
`);
	...
	const report = resolve("build/lists/php-wasm-zend-faults.json");
