/**
 * Installed wasm32 Lean lists in offline npm and Composer deployments.
 *
 * @file
 */
...
import { phpWasmListSettings, phpWasmListConsumer, phpWasmListRequest } from "./helpers/php-wasm-list-fixture.mjs";
...
test("installed PHP-Wasm lists preserve copied values in Node and Chromium", { skip: !enabled, timeout: 1_800_000 }, async t => {
	const reports = [];
	for(const path of ["ordinary-source", "reviewed-ir"])
	{
		const author = await mkdtemp(join(tmpdir(), "lean-bridge-php-wasm-list-author-"));
		const consumer = await mkdtemp(join(tmpdir(), "lean-bridge-php-wasm-list-consumer-"));
		...
		await saveLakeFile(projectRoot, "lean-bridge.exports.json", canonicalJson({ schemaVersion: 1
			, modules: ["Lists"], targets: { "php-wasm": phpWasmListSettings }
			, ...(path === "ordinary-source" ? { exports: listSignatures.map(entry => entry.name) } : {}) }));
		...
		t.diagnostic(`${path}: compiling the 27-export wasm32 list library`);
		...
		const installed = await installedPhpWasmCorpus({ t
			...
			, sourcePath: path
			, fixture: { settings: phpWasmListSettings
				, source: phpWasmListConsumer
				, request: phpWasmListRequest, removeHandoff: true } })
	...
	const reportPath = resolve(process.env.LEAN_BRIDGE_PHP_WASM_LIST_REPORT ?? "build/lists/php-wasm.json");
