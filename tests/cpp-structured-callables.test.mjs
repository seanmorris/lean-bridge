/**
 * Installed structured C++ callbacks and closures on both authoring paths.
 *
 * @file
 */
...
import { checkStructuredCppInstallation, parseStructuredCppResult } from "./helpers/cpp-structured-callable-install.mjs";

const enabled = process.env.LEAN_BRIDGE_CPP_STRUCTURED_CALLABLE_TEST === "1";
const signatures = ir => {
	const type = ref => {
...
test("installed C++ structured callbacks preserve nested values, exception cleanup and source-free closures", { skip: !enabled, timeout: 900_000 }, async t => {
	const reports = [], expected = signatures(structuredCallableReviewedIr());
	for(const path of ["ordinary-source", "reviewed-ir"])
	{
		const author = await mkdtemp(join(tmpdir(), "lean-bridge-cpp-structured-author-"));
		const consumer = await mkdtemp(join(tmpdir(), "lean-bridge-cpp-structured-consumer-"));
		...
		await saveLakeFile(projectRoot, "lean-bridge.exports.json", canonicalJson({ schemaVersion: 1
			, modules: ["Structured"]
			, targets: { cpp: { name: "structured", version: "1.0.0" } }
		...
		t.diagnostic(`${path}: compiling eight structured C++ callback/closure shapes`);
		...
		t.diagnostic(`${path}: testing relocated public C++ APIs without author sources or Lean`);
		const { command, ...observation } = await installCopiedConsumer({
			...
			, environment
			, fixture: { source: () => readFile("tests/fixtures/structured-callable-consumers/cpp.cpp", "utf8"), parseResult: parseStructuredCppResult } });
		assert.ok(command.startsWith(consumer));
		const receiptSha256 = sha256(await readFile(join(handoff, "package-set-receipt.json")));
		const safety = await checkStructuredCppInstallation({ consumer, handoff, packages: receipt.packages, observed: observation.result });
		reports.push({
			profile: "cpp"
			, path
			, ...observation
	...
	const reportPath = resolve(process.env.LEAN_BRIDGE_CPP_STRUCTURED_CALLABLE_REPORT ?? "build/structured-callables/cpp.json");
