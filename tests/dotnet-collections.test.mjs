/**
 * Original NuGet collection packages from ordinary Lean and reviewed source paths.
 *
 * @file
 */
...
import { installDotnetCollections } from "./helpers/dotnet-collection-install.mjs";
...
test("installed .NET arrays and records preserve values on both source paths", { skip: process.env.LEAN_BRIDGE_DOTNET_COLLECTION_TEST !== "1", timeout: 900_000 }, async t => {
	const reports = [], environment = nativeFixtureEnvironment(["dotnet"]);
	for(const path of ["ordinary-source", "reviewed-ir"])
	{
		const author = await mkdtemp(join(tmpdir(), "lean-bridge-dotnet-collection-author-"));
		const consumer = await mkdtemp(join(tmpdir(), "lean-bridge-dotnet-collection-consumer-"));
		t.after(() => Promise.all([author, consumer].map(root => rm(root, { recursive: true, force: true }))));
		const projectRoot = join(author, "project"), outputRoot = join(author, "release"), handoff = join(consumer, "handoff");
		await writeCollectionProject(projectRoot);
		await saveLakeFile(projectRoot, "lean-bridge.exports.json", canonicalJson({ schemaVersion: 1
			, modules: ["Collections"]
			, targets: { nuget: { name: "Lean.Collections", version: "1.0.0" } }
		...
		t.diagnostic(`${path}: compiling .NET collections`);
		...
		const sources = { api: await readFile(join(managed, "src/LeanBridge.Collections/Api.cs"), "utf8"), runtime: await readFile(join(managed, "src/LeanBridge.Collections/Runtime.cs"), "utf8") };
		const receipt = await copyPackageSetHandoff(outputRoot, handoff);
		await verifyPackageSetReceipt({ receiptPath: join(handoff, "package-set-receipt.json") });
		await rm(author, { recursive: true, force: true });
		t.diagnostic(`${path}: original offline NuGet package, typed callers, isolated failure probes and relocation`);
		const installed = await installDotnetCollections({ consumer, handoff, packages: receipt.packages, environment, sources, projection: compileCopiedDotnetModel(model.bindingIr) });
		reports.push({
			profile: "dotnet", path, signatures
			, ...installed
		...
		t.diagnostic(`${path}: ${installed.checks} public assertions, ${installed.calls} calls, ${installed.faults.native.checkpoints} injected failures`);
		await rm(consumer, { recursive: true, force: true });
	}
	assert.deepEqual(reports[0].nativeLibraries, reports[1].nativeLibraries);
	await saveLakeFile("build/collections", "dotnet.json", canonicalJson({ schemaVersion: 1, reports }));
});
