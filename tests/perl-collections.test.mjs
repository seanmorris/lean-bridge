/**
 * Installed CPAN arrays and records on independently specified ordinary and reviewed paths.
 *
 * @file
 */
...
import { preparePerlCollections } from "./helpers/perl-collection-install.mjs";

const enabled = process.env.LEAN_BRIDGE_PERL_COLLECTION_TEST === "1";
...
test("installed Perl arrays and records preserve copied values on both source paths", { skip: !enabled, timeout: 900_000 }, async t => {
	...
	{
		const author = await mkdtemp(join(tmpdir(), "lean-bridge-perl-collection-author-"));
		const consumer = await mkdtemp(join(tmpdir(), "lean-bridge-perl-collection-consumer-"));
		t.after(() => Promise.all([author, consumer].map(root => rm(root, { recursive: true, force: true }))));
		const projectRoot = join(author, "project"), outputRoot = join(author, "release"), handoff = join(consumer, "handoff");
		await writeCollectionProject(projectRoot);
		await saveLakeFile(projectRoot, "lean-bridge.exports.json", canonicalJson({ schemaVersion: 1
			, modules: ["Collections"]
			, targets: { cpan: { module: "LeanBridge::Collections", version: "1.000" } }
		...
		t.diagnostic(`${path}: compiling Perl arrays and records for ${perls.length} ABIs`);
		...
		for(const [index, perl] of perls.entries())
		{
			...
			prepared.push(await preparePerlCollections({ consumer: join(consumer, `abi-${index}`), handoff, packages: receipt.packages, perl, environment }));
		}
	...
	await saveLakeFile("build/collections", "perl.json", canonicalJson({ schemaVersion: 1, reports }));
});
