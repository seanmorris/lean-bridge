/**
 * Installed Java and Kotlin list values on ordinary and reviewed source paths.
 *
 * @file
 */
...
import { checkJvmListFaults } from "./helpers/jvm-list-faults.mjs";

const enabled = process.env.LEAN_BRIDGE_JVM_LIST_TEST === "1";
...
test("installed Java and Kotlin lists preserve copied values on both source paths", { skip: !enabled, timeout: 1_800_000 }, async t => {
	const reports = [], environment = nativeFixtureEnvironment(["java", "kotlin"]);
	for(const path of ["ordinary-source", "reviewed-ir"])
	{
		const directory = await mkdtemp(join(tmpdir(), "lean-bridge-jvm-list-author-"));
		const consumer = await mkdtemp(join(tmpdir(), "lean-bridge-jvm-list-consumer-"));
		...
		await saveLakeFile(projectRoot, "lean-bridge.exports.json", canonicalJson({ schemaVersion: 1
			, modules: ["Lists"]
			, targets: { maven: { name: "org.leanbridge:lists", version: "1.0.0" } }
		...
		t.diagnostic(`${path}: compiling the 27-export JVM list library`);
		...
		const faults = await checkJvmListFaults({ consumer, environment, sources, jar, projection: compileCopiedJvmModel(model.bindingIr) });
	...
	await saveLakeFile("build/lists", "jvm.json", canonicalJson({ schemaVersion: 1, reports }));
});
