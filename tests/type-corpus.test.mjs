	...
	return { library: library.id, profile, path: "ordinary-source"
		...
		, ...(wasm ? { runtimeArchive: { target: "npm", sha256: "e".repeat(64) }
			, rejection: { code: "component-adapter-hints-required"
				, exports: [...library.operations.slice(6, 7).map(name => `${library.module}.${name}`), library.pendingExport]
				, hints: [...library.operations.slice(6, 7).map(name => `hint:${library.module}.${name}:unsupported-parameter-type`), `hint:${library.pendingExport}:unsupported-result-type`].sort() }
		} : {})
...
const cFamilyValidationFixture = (library, profile) => ({
	...
	, pkgConfig: { version: "1.8.1", flags: ["-I/validator", ...profile === "cpp" ? ["-DBOOST_MP_STANDALONE"] : [], "-L/validator", "-Wl,-rpath,/validator", `-l${library.cModule}${profile === "c" ? "_gmp" : ""}`, ...profile === "c" ? ["-l:libgmp.so.10"] : []], manifestSha256: "f".repeat(64) }
...
for(const profile of ["c", "cpp"]) test(`${profile} corpus separates native numeric acceptance from compiler rejection`, () => {
	...
	assert.equal(results.filter(entry => entry.status === "rejected-at-compile-time").length, profile === "c" ? 26 : 28);
