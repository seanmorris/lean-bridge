...
test("WIT list conversions release partial values and scratch after injected failures", { skip: process.env.LEAN_BRIDGE_WIT_LIST_TEST !== "1", timeout: 120_000 }, async t => {
	const root = await mkdtemp(join(tmpdir(), "lean-bridge-wit-list-conversions-"));
	...
	const ir = witListFaultIr(), model = compileCopiedWitModel(ir), source = witListFaultSource(model);
	...
	const report = { schemaVersion: 1, synthetic: true, compilerOptions
		...
		, observation };
	const output = resolve("build/lists/wit-conversions.json");
