/**
 * Bind Ruby callable promotion to independent, source-hidden installed runs.
 *
 * @file
 */
...
import { rubyCallableSignatures } from "./helpers/ruby-callable-fixture.mjs";

test("Ruby callable evidence retains both source paths and all nineteen primitives", async () => {
	const record = JSON.parse(await readFile("docs/evidence/ruby-callables-20260919.json", "utf8"));
	for(const file of record.sources) assert.equal(file.sha256, sha256(await readFile(file.path)));
	assert.equal(record.consumerSha256, sha256(await readFile("tests/fixtures/callable-consumers/ruby.rb")));
	assert.deepEqual(record.signatures, rubyCallableSignatures);
	assert.deepEqual(record.executions.map(run => run.path), ["ordinary-source", "reviewed-ir"]);
	for(const run of record.executions)
	{
		assert.equal(run.profile, "ruby"); assert.ok(run.checks > 50_000);
		...
		assert.equal(run.packages[0].target, "rubygems");
