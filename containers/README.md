...
Before invoking Nix on a Git checkout, the entrypoint adds an exact source-path trust entry to the disposable container's Git configuration. Nix uses libgit2, so an environment-only `GIT_CONFIG_COUNT` override does not resolve this ownership mismatch. The cached component engine runs directly without Git or Nix; its read-only store mount can hide those executables from the image. The ownership check covers both paths and rejects missing or non-executable cached programs. Source mounts remain read-only; the entrypoint does not change host source ownership or trust settings.
