...
const makeFunction = ({ id, name, parameters, resultMode, effects, summary, details }) => ({
	...
	, kind: "function"
	, owner: null
	, overloadKey: `${name}(${parameters.map(() => "uint32").join(",")})`
	, typeParameters: []
	, receiver: null
	, parameters
	, result: uint32Result()
...
/**
 * Installs a bounded, generation-safe iterator fixture over the module’s private exports and exposes lifecycle diagnostics.
 *
 * @param module - Initialized runtime module whose private exports receive the iterator callbacks.
 */
export const installOverheadIteratorRuntime = module => {
	...
	return Object.freeze({
		setTransform:
			/**
       * Installs the callable applied to every sequence value emitted by the overhead iterator fixture.
       *
       * @param value - Callable transform invoked for each generated sequence value.
       */
			function(value) {
