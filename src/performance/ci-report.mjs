...
/**
 * Validates the performance manifest, evidence families, artifact identities, and CI job bindings, then emits the acceptance report.
 *
 * @param root0 - Manifest, measurements, jobs, and output identity used to build the CI report.
 * @param root0.manifest - Performance build manifest that binds source, workloads, and artifact identities.
 * @param root0.records - Evidence records keyed by the required performance family.
 * @param root0.jobs - CI job results whose roles and published evidence must match the workflow contract.
 * @param root0.artifact - Report artifact metadata recorded in the final evidence envelope.
 */
export const assemblePerformanceCiReport = ({ manifest, records, jobs = [], artifact }) => {
    ...
    return [family
    , record === null ? null : Object.freeze({
...
/**
 * Formats performance CI summary as stable output that the reproducible performance evidence pipeline can consume without heuristics.
 *
 * @param report - Validated performance CI report rendered into the human-readable summary.
 */
