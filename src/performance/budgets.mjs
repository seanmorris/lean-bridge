...
/**
 * Reports performance budget failures with stable machine-readable codes and structured diagnostic context.
 */
export class PerformanceBudgetError extends Error
{
	/**
   * Initializes the error used to report performance budget failures, preserving its code, message, and diagnostic context.
   *
...
/**
 * Flattens overhead, lifecycle, spatial, and scaling suites into a canonical, duplicate-free metric catalog.
 *
 * @param fork - Complete benchmark fork whose suites contain the measurements to catalog.
 */
...
/**
 * Validates eligible forks against one source, artifact catalog, workload identity, and metric shape before building an immutable baseline.
 *
 * @param root0 - Evidence and policy inputs required to authorize a reproducible performance baseline.
 * @param root0.methodology - Validated methodology that fixes sampling, statistics, and identity inputs.
 * @param root0.environmentReport - Accepted environment report proving the host is baseline-eligible.
 * @param root0.forks - Valid benchmark forks expected to share source, metrics, workloads, and artifacts.
 * @param root0.invalidForks - Rejected fork records retained to enforce the methodology’s failure allowance.
 * @param root0.rawForkFiles - Immutable source files corresponding one-to-one with the valid forks.
 * @param root0.reproductionCommand - Command a reviewer can run to reproduce the baseline evidence.
 */
...
/**
 * Derives performance budget only from explicit inputs so results remain reproducible in the reproducible performance evidence pipeline.
 *
 * @param root0 - Named inputs and dependency overrides used to derive performance budget.
 * @param root0.baseline - Accepted baseline evidence that supplies reference metrics and identities for comparison.
 * @param root0.baselinePath - Filesystem path to the baseline.
 * @param root0.baselineSha256 - Expected SHA-256 identity used to detect drift in baseline.
 * @param root0.reviewedBy - Reviewer identity recorded as budget provenance.
 * @param root0.rationale - Reviewer explanation justifying the budget ceilings and relative-change allowance.
 * @param root0.reviewedAt - RFC 3339 timestamp recording when the performance budget was approved.
 * @param root0.previousBudget - Earlier reviewed budget whose history and ceilings constrain the derived revision.
 */
...
/**
 * Validates performance budget against its closed contract before it enters the reproducible performance evidence pipeline.
 *
 * @param budget - Reviewed metric budget containing absolute ceilings and permitted relative change.
 */
...
/**
 * Compares performance candidate and returns bounded diagnostics for every material difference.
 *
 * @param root0 - Named inputs and dependency overrides used to compare performance candidate.
 * @param root0.baseline - Accepted baseline evidence that supplies reference metrics and identities for comparison.
 * @param root0.candidate - Candidate measurement evidence compared with the accepted baseline and budget.
 * @param root0.budget - Reviewed metric budget containing absolute ceilings and permitted relative change.
 * @param root0.baselineSha256 - Expected SHA-256 identity used to detect drift in baseline.
 */
...
/**
 * Computes the stable SHA-256 identity for performance record so the reproducible performance evidence pipeline can detect byte drift.
 *
 * @param value - Performance record whose canonical JSON bytes determine the returned digest.
 */
