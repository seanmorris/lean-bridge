/**
 * Implements the lifecycle module in the performance subsystem.
 *
 * @file
 */
...
const createRuntime = async (options = {}) => {
	...
	const libraries = createLibraryLoader(module, {
		...
		, onCallbackFrame:
			/**
       * Tracks the deepest callback frame observed by the suite before forwarding the event to the caller’s hook.
       *
...
/**
 * Runs lifecycle stability suite and returns a structured result suitable for the reproducible performance evidence pipeline.
 *
 * @param options - Round sizes, lifecycle sample counts, garbage-collection policy, and callback observer overrides.
 */
