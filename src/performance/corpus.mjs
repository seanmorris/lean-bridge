...
/**
 * Reports performance corpus failures with stable machine-readable codes and structured diagnostic context.
 */
export class PerformanceCorpusError extends Error
{
	/**
   * Initializes the error used to report performance corpus failures, preserving its code, message, and diagnostic context.
   *
...
/**
 * Validates performance corpus against its closed contract before it enters the reproducible performance evidence pipeline.
 *
 * @param value - Corpus document whose datasets, vectors, steps, and expectations must satisfy the closed schema.
 */
...
/**
 * Computes a stable content identity for performance corpus so the reproducible performance evidence pipeline can reject drift.
 *
 * @param value - Validated corpus document whose canonical bytes determine the returned digest.
 */
...
/**
 * Provides a deterministic spatial index used as the correctness reference for performance workloads.
 */
class ReferenceIndex
{
	/**
   * Retains copied reference points used by deterministic spatial queries.
   */
	#points;
	/**
   * Prevents spatial operations after the reference index has released its state.
   */
	#disposed = false;

	/**
   * Builds an isolated reference index from copied input points.
   *
   * @param points - Spatial points copied into the workload or index fixture.
   */
	constructor(points)
	{
	...
	/**
   * Rejects spatial operations after disposal and returns the active point collection otherwise.
   */
	#live()
	{
   ...
   *
   * @param query - Copied query point used to find the nearest indexed value.
   */
	nearest(query)
	{
		this.#live();
   ...
   *
   * @param minimum - Inclusive lower coordinate bounds for the range query.
   * @param maximum - Inclusive upper coordinate bounds for the range query.
   */
   ...
   *
   * @param point - Copied spatial point supplied to the reference index.
   */
	insert(point)
	{
		this.#live();
...
/**
 * Runs performance corpus vectors and returns a structured result suitable for the reproducible performance evidence pipeline.
 *
 * @param value - Corpus document containing the datasets and vectors to execute against the reference runner.
 */
