/**
 * Implements the publish manifest module in the release subsystem.
 *
 * @file
 */
...
/**
 * Reports publish manifest failures with stable machine-readable codes and structured diagnostic context.
 */
export class PublishManifestError extends Error
{
	/**
   * Initializes the error used to report publish manifest failures, preserving its code, message, and diagnostic context.
   *
...
/**
 * Formats a package coordinate using the syntax required by its registry ecosystem.
 *
 * @param item - Package name, version, and ecosystem from the canonical publication manifest.
 */
...
/**
 * Returns the closed publication endpoint, operation, and credential policy for a supported ecosystem.
 *
 * @param ecosystem - Registry ecosystem whose publication policy is required.
 */
...
/**
 * Validates publish manifest against its closed contract before it enters the deterministic release and independent-verification pipeline.
 *
...
/**
 * Selects authorized registry-ready packages and builds an ordered, identity-bound manifest for publication or retention.
 *
 * @param root0 - Named inputs and dependency overrides used to create publish manifest.
 * @param root0.authorization - Verified release authorization that bounds the packages and artifacts eligible for publication.
 * @param root0.authorizationSha256 - Expected SHA-256 identity used to detect drift in authorization.
 * @param root0.publicationIndex - Zero-based index identifying the publication.
 * @param root0.requestedTargets - User-requested publication targets normalized and checked against authorization.
 */
...
/**
 * Writes publish manifest in deterministic form with the metadata required by the deterministic release and independent-verification pipeline.
 *
 * @param root0 - Named inputs and dependency overrides used to write publish manifest.
 * @param root0.gateRoot - Filesystem root containing the gate.
 * @param root0.requestedTargets - User-requested publication targets normalized and checked against authorization.
...
/**
 * Verifies publish manifest against recorded identities and rejects any drift before the deterministic release and independent-verification pipeline proceeds.
 *
 * @param root0 - Named inputs and dependency overrides used to verify publish manifest.
