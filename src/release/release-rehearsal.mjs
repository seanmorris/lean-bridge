...
/**
 * Reports publication index failures with stable machine-readable codes and structured diagnostic context.
 */
export class PublicationIndexError extends Error
{
	/**
   * Initializes the error used to report publication index failures, preserving its code, message, and diagnostic context.
   *
...
/**
 * Validates publication index against its closed contract before it enters the deterministic release and independent-verification pipeline.
 *
...
/**
 * Parses publication index and validates the resulting closed representation before returning it to the deterministic release and independent-verification pipeline.
 *
...
/**
 * Builds every eligible ecosystem package and records omissions, artifact identities, and backend plans in a rehearsal manifest.
 *
 * @param root0 - Verified bundle and empty destination used for the isolated rehearsal.
 * @param root0.bundleRoot - Canonical release bundle whose package mappings will be rehearsed.
 * @param root0.outputRoot - Empty directory that receives package projections and the rehearsal manifest.
 */
