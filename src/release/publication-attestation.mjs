...
/**
 * Reports publication attestation failures with stable machine-readable codes and structured diagnostic context.
 */
export class PublicationAttestationError extends Error
{
	/**
   * Initializes the error used to report publication attestation failures, preserving its code, message, and diagnostic context.
   *
...
/**
 * Validates an Ed25519 public key and builds the canonical single-signer DSSE publication policy.
 *
 * @param root0 - Named inputs and dependency overrides used to create publication signer policy.
 * @param root0.identity - Expected signer identity recorded in the publication policy.
 * @param root0.publicKeyPem - PEM-encoded public key used to verify signatures from the declared signer.
 */
...
/**
 * Validates publication signer policy against its closed contract before it enters the deterministic release and independent-verification pipeline.
 *
...
/**
 * Computes the stable SHA-256 identity for publication signer policy so the deterministic release and independent-verification pipeline can detect byte drift.
 *
 * @param policy - Closed policy document that governs authorization and validation.
 */
...
/**
 * Binds authorized manifests, packages, locks, assurance files, SBOMs, and provenance into the signed publication statement.
 *
 * @param root0 - Named inputs and dependency overrides used to create publication statement.
...
/**
 * Encodes a payload with its type and byte lengths using the DSSE version 1 preauthentication format.
 *
 * @param type - UTF-8 DSSE payload type included in the signed byte sequence.
 * @param payload - Buffer-compatible statement bytes protected by the signature.
 */
...
/**
 * Authorizes publication only after its identities, policy, and evidence satisfy the deterministic release and independent-verification pipeline.
 *
 * @param root0 - Named inputs and dependency overrides used to authorize publication.
 * @param root0.verified - Verified publication closure binding the manifest, authorization, artifacts, and source identities.
 * @param root0.policy - Closed policy document that governs authorization and validation.
...
/**
 * Verifies publication attestation against recorded identities and rejects any drift before the deterministic release and independent-verification pipeline proceeds.
 *
 * @param root0 - Named inputs and dependency overrides used to verify publication attestation.
 * @param root0.policy - Closed policy document that governs authorization and validation.
 * @param root0.envelope - DSSE envelope whose payload type, signature, and statement identity are verified.
 * @param root0.verified - Verified publication closure binding the manifest, authorization, artifacts, and source identities.
 */
