...
/**
 * Reports registry transaction failures with stable machine-readable codes and structured diagnostic context.
 */
export class RegistryTransactionError extends Error
{
	/**
   * Initializes the error used to report registry transaction failures, preserving its code, message, and diagnostic context.
   *
...
/**
 * Validates registry transaction against its closed contract before it enters the deterministic release and independent-verification pipeline.
 *
 * @param state - Candidate lifecycle state requested by the transition.
 */
...
/**
 * Creates a resumable publisher that persists every target transition, scopes credential access, and rolls back failed registry writes.
 *
 * @param root0 - Named inputs and dependency overrides used to create registry transaction publisher.
 * @param root0.adapters - Registry adapter implementations keyed by the ecosystems the transaction can publish.
 * @param root0.transactionFile - Durable transaction-state path updated after each registry action.
