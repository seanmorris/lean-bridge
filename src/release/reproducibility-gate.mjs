/**
 * Implements the reproducibility gate module in the release subsystem.
 *
 * @file
 */
...
/**
 * Reports reproducibility gate failures with stable machine-readable codes and structured diagnostic context.
 */
export class ReproducibilityGateError extends Error
{
	/**
   * Initializes the error used to report reproducibility gate failures, preserving its code, message, and diagnostic context.
   *
...
/**
 * Prepares clean git sources in an isolated, deterministic form for the deterministic release and independent-verification pipeline.
 *
 * @param root0 - Named inputs and dependency overrides used to prepare clean git sources.
...
export const prepareCleanGitSources = async ({ projectRoot, scratchRoot, runner = processBuildRunner }) => {
	...
	{
		// Falling back to the portable local repository identity is intentional.
	}
	const roots = [];
...
/**
 * Runs reproducibility gate and returns a structured result suitable for the deterministic release and independent-verification pipeline.
 *
 * @param root0 - Named inputs and dependency overrides used to run reproducibility gate.
 * @param root0.projectRoot - Filesystem root containing the project.
...
/**
 * Validates release authorization against its closed contract before it enters the deterministic release and independent-verification pipeline.
 *
 * @param authorization - Verified release authorization that bounds the packages and artifacts eligible for publication.
 */
...
/**
 * Verifies release authorization against recorded identities and rejects any drift before the deterministic release and independent-verification pipeline proceeds.
 *
 * @param root0 - Named inputs and dependency overrides used to verify release authorization.
 * @param root0.authorizationRoot - Filesystem root containing the authorization.
 * @param root0.candidateRoot - Filesystem root containing the candidate.
 */
