		...
		record.promise = (async () => {
			...
			else if(copied)
			{
				...
				if([componentRecordAbi, componentCompoundAbi, componentNominalAbi].includes(descriptor.privateAbi.version))
				{
					...
					if(descriptor.privateAbi.version === componentNominalAbi && (!module._bridge_nominal_abi || module._bridge_nominal_abi() !== 1)) throw new Error("Shared runtime lacks the component nominal ABI; rebuild it");
...
{ module._free(name); }
				...
				const calls = new Map(descriptor.privateAbi.exports.map(abi => {
					const copiedCall = copied ? compileComponentCopiedCall(module, operations.get(abi.bindingId), abi, poison, descriptor.privateAbi.version, descriptor.privateAbi.types ?? descriptor.privateAbi.records) : null;
