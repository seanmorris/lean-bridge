...
import { validatePackageMetadata } from "../analyze/package-metadata.mjs";
 ...
 * @param metadata - Optional validated declaration from that package.
 */
export const isSourceNotice = (path, metadata = {}) => safePath(path) && (metadata.licenseFiles?.includes(path) || path.split("/").slice(0, -1).some(part => /^licenses$/i.test(part))
 ...
 * @param metadata - Optional validated declaration from that package.
 */
export const isSourceLicense = (path, metadata = {}) => isSourceNotice(path, metadata) && (metadata.licenseFiles?.includes(path)
	|| (!/^(?:notices?|copyright)(?:[._-].+)?$/i.test(path.split("/").at(-1))
		&& (path.split("/").slice(0, -1).some(part => /^licenses$/i.test(part))
		|| /^(?:licen[cs]es?|copying)(?:[._-].+)?$/i.test(path.split("/").at(-1)))));

/**
 * Bind per-package declarations to captured source bytes, including dependencies.
 *
 * @param source - Exact configuration text, or null when no file was captured.
 * @param inputs - That package's original source inventory.
 */
export const sourceNoticeMetadata = (source, inputs) => {
	...
	if(input ? typeof source !== "string" || !source.isWellFormed() || Buffer.byteLength(source) !== input.bytes || sha256(source) !== input.sha256 : source !== null)
		throw new Error("Source notice declaration differs from captured configuration");
	const metadata = validatePackageMetadata(source === null ? {} : JSON.parse(source).package ?? {});
	for(const path of metadata.licenseFiles ?? [])
		if(!inputs.some(file => file.path === path && file.bytes > 0)) throw new Error(`Declared license file is missing or empty: ${path}`);
	return metadata;
};
...
export const captureSourceNotices = async ({ projectRoot, projectName, inputs, sourceTreeSha256, snapshot, snapshotRoot }) => {
	...
	const capture = async (name, source, files, root) => {
		const notices = [];
		const configurationSource = files.some(file => file.path === "lean-bridge.exports.json") ? (await regularBytes(join(root, "lean-bridge.exports.json"))).toString("utf8") : null;
		const metadata = sourceNoticeMetadata(configurationSource, files);
		for(const file of files.filter(file => isSourceNotice(file.path, metadata)).sort((a, b) => a.path < b.path ? -1 : 1))
		{
			...
			if(metadata.licenseFiles?.includes(file.path) && !bytes.toString("utf8").trim()) throw new Error(`Declared license file is empty: ${file.path}`);
		...
		packages.push({ name, source, configurationSource, notices });
	};
	...
	const document = { schemaVersion: 2, kind: "lean-bridge-source-notices", packages };
...
export const readVerifiedSourceNotices = async (root, sourceIdentity) => {
	...
	if(!closed(document, ["schemaVersion", "kind", "packages"]) || ![1, 2].includes(document.schemaVersion) || document.kind !== "lean-bridge-source-notices"
	...
	{
		if(!closed(pkg, ["name", "source", "notices", ...(document.schemaVersion === 2 ? ["configurationSource"] : [])]) || typeof pkg.name !== "string" || !pkg.name || !Array.isArray(pkg.notices)) throw new Error("Invalid source notice package");
		...
		const metadata = document.schemaVersion === 2 ? sourceNoticeMetadata(pkg.configurationSource, inputs) : {};
		const identity = ({ path, bytes, sha256 }) => ({ path, bytes, sha256 });
		const expected = inputs.filter(file => isSourceNotice(file.path, metadata)).sort((a, b) => a.path < b.path ? -1 : 1).map(identity);
		...
		{
			if(!closed(notice, ["path", "bytes", "sha256", "payload"]) || !isSourceNotice(notice.path, metadata) || notice.path <= previous
			...
			if(metadata.licenseFiles?.includes(notice.path) && !contents.toString("utf8").trim()) throw new Error(`Declared license file is empty: ${notice.path}`);
