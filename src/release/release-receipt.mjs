...
/**
 * Reports release receipt failures with stable machine-readable codes and structured diagnostic context.
 */
export class ReleaseReceiptError extends Error
{
	/**
   * Initializes the error used to report release receipt failures, preserving its code, message, and diagnostic context.
   *
...
/**
 * Derives consumer installation commands for a published package or extraction commands for a retained archive.
 *
 * @param target - Verified publication target containing its ecosystem, coordinate, operation, and archives.
 */
...
/**
 * Binds the authorized plan, completed registry transaction, publication attestation, install commands, and source locks into a receipt statement.
 *
 * @param root0 - Named inputs and dependency overrides used to create release receipt statement.
 * @param root0.verified - Verified publication closure binding the manifest, authorization, artifacts, and source identities.
 * @param root0.transaction - Completed registry transaction whose target outcomes and identity are recorded or verified.
 * @param root0.transactionPath - Filesystem path to the transaction.
 * @param root0.transactionSha256 - Expected SHA-256 identity used to detect drift in transaction.
 * @param root0.publicationAttestation - Verified publication attestation binding the release statement to an accepted signer.
 * @param root0.policy - Closed policy document that governs authorization and validation.
 */
...
/**
 * Authorizes release receipt only after its identities, policy, and evidence satisfy the deterministic release and independent-verification pipeline.
 *
 * @param options - Verified publication closure, transaction evidence, signer policy, and signer required for DSSE authorization.
 */
...
/**
 * Validates release receipt against its closed contract before it enters the deterministic release and independent-verification pipeline.
 *
...
/**
 * Verifies release receipt document against recorded identities and rejects any drift before the deterministic release and independent-verification pipeline proceeds.
 *
 * @param root0 - Named inputs and dependency overrides used to verify release receipt document.
 * @param root0.receipt - Release receipt whose evidence and identities are verified.
 * @param root0.policy - Closed policy document that governs authorization and validation.
 * @param root0.verified - Verified publication closure binding the manifest, authorization, artifacts, and source identities.
 * @param root0.transaction - Completed registry transaction whose target outcomes and identity are recorded or verified.
 * @param root0.transactionPath - Filesystem path to the transaction.
...
/**
 * Verifies release receipt against recorded identities and rejects any drift before the deterministic release and independent-verification pipeline proceeds.
 *
 * @param root0 - Named inputs and dependency overrides used to verify release receipt.
...
/**
 * Writes release receipt in deterministic form with the metadata required by the deterministic release and independent-verification pipeline.
 *
 * @param root0 - Named inputs and dependency overrides used to write release receipt.
 * @param root0.verified - Verified publication closure binding the manifest, authorization, artifacts, and source identities.
 * @param root0.transactionResult - Completed publisher result containing the canonical transaction document and identity.
 * @param root0.publicationAttestation - Verified publication attestation binding the release statement to an accepted signer.
 * @param root0.policy - Closed policy document that governs authorization and validation.
 * @param root0.signer - Opaque signer provider used to produce a signature inside the authorized boundary.
 * @param root0.signal - Abort signal used to cancel the operation.
 * @param root0.receiptFile - Destination path for the canonical signed release receipt.
 */
