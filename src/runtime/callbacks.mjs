...
/**
 * Reports callback runtime failures with stable machine-readable codes and structured diagnostic context.
 */
export class CallbackRuntimeError extends Error
{
	/**
   * Initializes the error used to report callback runtime failures, preserving its code, message, and diagnostic context.
   *
...
/**
 * Owns callback handles, nested invocation frames, and deterministic cleanup for one runtime instance.
 */
export class CallbackRegistry
{
	/**
   * Initializes callback storage with a bounded handle space and an optional frame observer.
   *
   ...
   * @param root0.onFrame - Observer notified when callback frames enter or leave the registry.
   */
   ...
   */
	...
	/**
   * Emits an immutable lifecycle event to the configured observer when one is installed.
   *
   * @param event - Lifecycle event being observed or applied.
   * @param frame - Active callback frame that records nested invocation state.
   * @param details - Structured diagnostic fields associated with the failure.
   */
	...
	/**
   * Rejects work after the registry has entered its terminal shutdown state.
   */
	requireOpen()
	{
		if(this.state !== "open")
   ...
   */
	...
	/**
   * Resolves a previously retained callback token without consuming its handle.
   *
   * @param token - Generation-safe handle identifying the live native entry.
   */
	...
	/**
   * Allocates a stable handle for a callback and records the lifecycle policy governing it.
   *
   ...
   */
	...
	/**
   * Removes a callback from the forward identity index when its final handle retires.
   *
   ...
   */
	...
	/**
   * Releases one retained handle after confirming that its generation and policy still match.
   *
   * @param token - Generation-safe handle identifying the live native entry.
   * @param plan - Validated plan that defines the allowed operation and targets.
   */
	...
	/**
   * Pushes a callback frame after validating depth, re-entry, and invocation policy.
   *
   * @param entry - Resolved registry entry whose lifecycle is being updated.
   */
	enterFrame(entry)
	...
	/**
   * Unwinds a callback frame and applies its once-only or self-disposal policy.
   *
   * @param frame - Active callback frame that records nested invocation state.
   * @param entry - Resolved registry entry whose lifecycle is being updated.
   * @param slot - Registry slot associated with the generation-safe handle.
   ...
   */
	...
	/**
   * Invokes a callback that was retained earlier without changing its ownership state.
   *
   * @param token - Generation-safe handle identifying the live native entry.
   * @param args - Arguments forwarded through the validated adapter.
   */
	...
	/**
   * Invokes a native operation while enforcing callback re-entry and frame-depth policy.
   *
   * @param token - Generation-safe handle identifying the live native entry.
   * @param plan - Validated plan that defines the allowed operation and targets.
   * @param operation - Native callback invocation performed while the validated frame is active.
   * @param args - Arguments forwarded through the validated adapter.
   */
	...
	/**
   * Rejects native re-entry when the active callback policy forbids it.
   */
	beforeNativeCall()
	{
	...
	/**
   * Executes one resolved callback entry and records its outcome before unwinding the frame.
   *
   * @param entry - Resolved registry entry whose lifecycle is being updated.
   * @param slot - Registry slot associated with the generation-safe handle.
   * @param args - Arguments forwarded through the validated adapter.
