/**
 * Implements the consumer support module in the adoption subsystem.
 *
 * @file
 */
...
/**
 * Reports consumer support failures with stable machine-readable codes and structured diagnostic context.
 */
export class ConsumerSupportError extends Error
{
	/**
   * Initializes the error used to report consumer support failures, preserving its code, message, and diagnostic context.
   *
...
/**
 * Validates consumer support against its closed contract before it enters the documented consumer acceptance workflow.
 *
...
/**
 * Loads consumer support, verifies its structure and identity, and returns it to the documented consumer acceptance workflow.
 *
...
/**
 * Validates consumer result against its closed contract before it enters the documented consumer acceptance workflow.
 *
 * @param result - Candidate result whose closed shape and outcome semantics are validated.
 * @param contract - Validated contract that defines the expected consumers or cross-language semantics.
 * @param root0 - Named inputs and dependency overrides used to validate consumer result.
 * @param root0.allowFailed - Whether a failed consumer observation may be returned for aggregate reporting instead of rejected immediately.
 */
...
/**
 * Validates one CI result per declared consumer and assembles the closed pass-or-fail support report.
 *
 * @param root0 - Support contract and corresponding downstream CI results.
 * @param root0.contract - Declared support states and requirements for every downstream consumer.
 * @param root0.results - CI observations that must cover each declared consumer exactly once.
 */
...
/**
 * Renders the consumer support report as Markdown tables with execution, packaging, blocker, and measurement context.
 *
 * @param report - Validated support report produced from the declared contract and CI results.
 */
