...
/**
 * Reports managed binding generation failures with stable machine-readable codes and structured diagnostic context.
 */
export class ManagedBindingGenerationError extends Error
{
	/**
   * Initializes the error used to report managed binding generation failures, preserving its code, message, and diagnostic context.
   *
...
/**
 * Compiles managed alpha model into the explicit representation consumed by the generated native-language binding pipeline.
 *
 * @param ir - Binding IR document that defines the source types and operations.
 * @param target - Managed runtime target that selects capabilities and projection constraints.
 */
...
/**
 * Builds the closed managed-binding manifest that records generator identity, projected files, exports, features, and capability gaps.
 *
 * @param root0 - Validated model and generated file inventories written into the manifest.
 * @param root0.model - Managed projection model containing component identity, target, features, and gaps.
 * @param root0.generator - Stable generator name recorded with the manifest schema version.
 * @param root0.files - Complete generated file inventory, excluding the manifest itself.
 * @param root0.publicFiles - Generated files consumers may import or compile directly.
 * @param root0.internalFiles - Generated implementation files kept outside the public surface.
 * @param root0.packageFiles - Registry packaging files added by the target-specific generator.
 */
