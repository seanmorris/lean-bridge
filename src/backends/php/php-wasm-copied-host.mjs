...
const protocol = "php-wasm-copied-startup-v1";
...
export const createPhpWasmCopiedDescriptor = (runtime, component, assets) => {
	if(!hash(runtime?.identity) || !hash(runtime?.loaderIdentity)
		...
		|| ![assets?.library, assets?.api, assets?.native].every(url => url instanceof URL))
	...
	const prepare = php => {
		...
		if(php.binary !== undefined) reject("php-wasm-host-already-started", "Register copied descriptors in sharedLibs before PHP starts");
		if((args.dynamicLibs ?? []).flat().some(item => item?.getLibs === getLibs))
			reject("unsupported-php-wasm-loading", "This copied package supports sharedLibs startup loading, not dynamicLibs");
		...
		if(!entry)
		{
			entry = { fingerprint, librariesEmitted: false, filesEmitted: false };
	...
	const getLibs = php => {
		const { state, entry } = prepare(php), libraries = [];
		...
		{
			libraries.push({ name: definition.library, url: new URL(assets.library), ini: true });
			entry.librariesEmitted = true;
		}
		return libraries;
	};
	const getFiles = php => {
		const { entry } = prepare(php);
		if(entry.filesEmitted) return [];
		entry.filesEmitted = true;
		return [["src/Api.php", assets.api], ["src/Internal/Native.php", assets.native]].map(([path, url]) => ({
			path: `/vendor/${definition.composer}/${path}`
			, url: new URL(url)
		}));
	};
	return Object.freeze({ getLibs, getFiles, autoload, extensions: Object.freeze({ getLibs }) });
};
