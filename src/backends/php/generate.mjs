...
{
	/**
   * Initializes the error used to report PHP binding generation failures, preserving its code, message, and diagnostic context.
   *
...
const emitCallback = (ir, projection, type) => {
	const imports = [
		`${projection.package.namespace}\\Internal\\Identity`
		, `${projection.package.namespace}\\Internal\\Runtime`
		, `${projection.package.namespace}\\Internal\\TransportError`
...
export const compilePhpPackageModel = (ir, options = {}) => {
	...
	if(ordinary)
		return Object.freeze({ ir, copied: compileCopiedPhpModel(ir, { lists: true, variants: true }) });
	validateCoverage(ir);
...
export const renderPhpPackageLayout = model => {
	...
	for(const type of ir.types)
	{
		...
		if(type.kind === "callback") addPublic(`src/${type.name}.php`, emitCallback(ir, projection, type));
	}
