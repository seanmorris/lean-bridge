...
const names = model => ["LeanBridgeError", ...model.surface.callbacks.size ? ["LeanClosure"] : [], ...model.surface.copies.filter(copy => copy.record).map(copy => copy.publicName), ...model.surface.functions.map(fn => fn.field)];
...
from . import ${names(model).filter(name => ["LeanBridgeError", "LeanClosure"].includes(name) || model.surface.copies.some(copy => copy.publicName === name)).join(", ")}
