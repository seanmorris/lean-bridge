...
const specializations = declaration => {
  ...
  if (
    ...
    fail("unsupported-generic", `${declaration.id} cannot use finite Python specialization`, {
  ...
  return branches.map((branch, index) => {
    ...
    if (ids.has(branch.id)) fail("duplicate-generic-specialization", `${declaration.id} repeats ${branch.id}`);
    ids.add(branch.id);
    const guard = branch.type.name === "bool" ? "bool" :
      branch.type.name === "string" ? "string" :
        branch.type.name === "bytes" ? "bytes" :
          new Set(["float32", "float64"]).has(branch.type.name) ? "float" : "int";
    if (guards.has(guard)) {
      fail("ambiguous-generic-specialization", `${declaration.id} maps ${guards.get(guard)} and ${branch.id} to Python ${guard}`, {
        declaration: declaration.id,
        guard,
      });
    }
  ...
  for (const ref of collectRefs(ir)) {
    ...
    if (ref.kind === "primitive") {
      ...
      if (n === "unit") lines.push("    if value is not None: raise TypeError(f\"{path} must be None\")");
        ...
        const bits = Number(n.slice(4));
        lines.push(`    if type(value) is not int or value < 0 or value > ${(2n ** BigInt(bits) - 1n).toString()}: raise ValueError(f\"{path} must be ${n}\")`);
      } else if (n.startsWith("int") && n !== "int") {
        const bits = Number(n.slice(3));
        lines.push(`    if type(value) is not int or value < ${(-(2n ** BigInt(bits - 1))).toString()} or value > ${(2n ** BigInt(bits - 1) - 1n).toString()}: raise ValueError(f\"{path} must be ${n}\")`);
