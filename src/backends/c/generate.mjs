...
const finiteSpecializations = declaration => {
  ...
  if (
    ...
    fail("unsupported-generic", `${declaration.id} cannot be monomorphized by the C POC`, {
  ...
  return branches.map((branch, index) => {
    if (
      ...
      (primitiveCType(branch.type.name) === null && !isDynamicPrimitive(branch.type.name))
    ) {
    ...
    ids.add(branch.id);
    return Object.freeze({ id: branch.id, type: structuredClone(branch.type) });
  });
};
