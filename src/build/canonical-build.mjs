...
/**
 * Detects build backend from explicit host capabilities and reports the selection to the isolated component build pipeline.
 *
 * @param root0 - Named inputs and dependency overrides used to detect build backend.
...
export const detectBuildBackend = async ({
	...
	if(requested === "docker" && !docker.available)
	{
		fail("docker-unavailable", "Docker was explicitly selected but its daemon is unavailable", {
			hint: "Install and start Docker, or set LEAN_BRIDGE_BUILD_BACKEND=nix."
			, details: available
		});
	}
	if(requested === "nix" && !nix.available)
	{
		fail("nix-unavailable", "Native Nix was explicitly selected but is unavailable", {
			hint: "Install Nix, or set LEAN_BRIDGE_BUILD_BACKEND=docker."
			, details: available
		});
	}
	...
	fail("build-tools-unavailable", "Lean Bridge requires Docker or native Nix to run the canonical flake", {
		hint: "Install and start Docker, or install Nix and run with LEAN_BRIDGE_BUILD_BACKEND=nix."
		, details: available
	});
};

/**
 * Loads builder manifest, verifies its structure and identity, and returns it to the isolated component build pipeline.
 *
 ...
 * @param root0.selection - Resolved build backend selection containing its command, environment, and provenance.
 * @param root0.runner - Process runner used for isolated external commands.
 * @param root0.environment - Environment variables used to resolve tools and policy.
 */
...
/**
 * Runs docker component engine and returns a structured result suitable for the isolated component build pipeline.
 *
 * @param root0 - Named inputs and dependency overrides used to run docker component engine.
 * @param root0.engineRoot - Filesystem root containing the engine.
 * @param root0.inputRoot - Filesystem root containing the input.
 * @param root0.requestPath - Filesystem path to the request.
 * @param root0.outputRoot - Filesystem root containing the output.
...
export const runDockerComponentEngine = async ({
	...
	await runner.capture({
		...
		, args: [
			...
			, "--mount"
			, `type=bind,source=${resolve(engineRoot)},target=/workspace/engine,readonly`
			, "--mount"
			, `type=bind,source=${resolve(inputRoot)},target=/workspace/component,readonly`
			, "--mount"
			, `type=bind,source=${resolve(dirname(requestPath))},target=/workspace/request,readonly`
			, "--mount"
			, `type=bind,source=${resolve(dirname(outputRoot))},target=/workspace/output`
			...
			, "--env"
			, `LEAN_BRIDGE_OUTPUT_UID=${typeof process.getuid === "function" ? process.getuid() : 0}`
			, "--env"
			, `LEAN_BRIDGE_OUTPUT_GID=${typeof process.getgid === "function" ? process.getgid() : 0}`
			, image, "component"
		]
		, cwd: resolve(engineRoot)
		, timeoutMs: 60 * 60 * 1000
...
/**
 * Builds pinned builder image from validated inputs with deterministic output suitable for the isolated component build pipeline.
 *
 * @param root0 - Named inputs and dependency overrides used to build pinned builder image.
 * @param root0.projectRoot - Filesystem root containing the project.
 * @param root0.dockerCommand - Docker-compatible executable used to build the pinned builder image.
 * @param root0.runner - Process runner used for isolated external commands.
 * @param root0.builder - Pinned builder implementation or image metadata used to produce release artifacts.
...
export const buildPinnedBuilderImage = async ({
	...
	await runner.capture({
		...
		, args: [
			...
			, "--build-arg"
			, `SOURCE_DATE_EPOCH=${resolvedBuilder.manifest.sourceDateEpoch}`
			, "--tag", image
...
/**
 * Builds canonical project from validated inputs with deterministic output suitable for the isolated component build pipeline.
 *
 * @param root0 - Named inputs and dependency overrides used to build canonical project.
 * @param root0.projectRoot - Filesystem root containing the project.
 * @param root0.engineRoot - Filesystem root containing the engine.
 * @param root0.outputRoot - Filesystem root containing the output.
 * @param root0.environment - Environment variables used to resolve tools and policy.
...
export const buildCanonicalProject = async ({
	...
	const sourceC = root !== engine && targets.some(target => ["c", "cpp", "nuget", "maven", "rubygems", "wit-wasi", "pypi", "cargo"].includes(target))
	...
	if(sourceC || targets.includes("cpan") || targets.includes("perl"))
	{
		...
		if(normalized.some(target => !["npm", "cpan", "c", "cpp", "nuget", "maven", "rubygems", "wit-wasi", "pypi", "cargo"].includes(target)))
			fail("invalid-package-targets", "Combined ordinary builds support npm, cpan, c, cpp, nuget, maven, rubygems, wit-wasi, pypi, and cargo targets");
...
/**
 * Expresses the resolved output directory as a platform-neutral path relative to the project root.
 *
 * @param root0 - Project and output roots used to compute the portable relative path.
 * @param root0.projectRoot - Base project directory for the relative-path calculation.
 * @param root0.outputRoot - Build output directory to express relative to the project.
 */
