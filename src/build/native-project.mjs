	...
	try
	{
		...
		const built = await buildNativeComponent({ projectRoot: project
			...
			, validateModel: model => {
				...
				const cSurface = compilePrimitiveCSurface(model.bindingIr, { compounds: targets.every(target => ["c", "cpp", "pypi", "cargo", "nuget", "maven"].includes(target)), callables: cTargets.every(target => ["c", "cpp", "pypi", "rubygems", "cargo", "nuget", "maven", "php-native", "wit-wasi"].includes(target)) });
