...
export const projectNativeCFamily = async ({ working, nativeRoot, runtimeRoot, leanPrefix, targets, settings = {}, environment = process.env, signal }) => {
	...
	const surface = compilePrimitiveCSurface(model.bindingIr, { compounds: targets.every(target => ["c", "cpp", "pypi", "cargo", "nuget", "maven"].includes(target)), callables: targets.every(target => ["c", "cpp", "pypi", "rubygems", "cargo", "nuget", "maven", "php-native", "wit-wasi"].includes(target)) }), p = surface.prefix;
