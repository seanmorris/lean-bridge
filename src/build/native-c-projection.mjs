...
export const projectNativeCFamily = async ({ working, nativeRoot, runtimeRoot, leanPrefix, targets, settings = {}, environment = process.env, signal }) => {
	...
	const surface = compilePrimitiveCSurface(model.bindingIr, { callables: targets.every(target => ["c", "cpp", "pypi", "rubygems", "cargo", "nuget"].includes(target)) }), p = surface.prefix;
