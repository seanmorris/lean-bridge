/**
 * Compiler-authenticated native graph carriers, independent of Wasm framing.
 *
 * @file
 */
 ...
 * Native artifacts retain neither a Wasm dispatch name nor a wasm32 wire ABI.
 *
 * @param model - Reconstructed native copied-graph component model.
 */
export const nativeGraphCarrierAbi = model => {
	if(model.profile !== "native-library-v1" || model.pointerBits !== 64 || model.byteOrder !== "little"
		|| ![4, 5].includes(model.schemaVersion) || canonicalJson(model.copiedGraph) !== canonicalJson(descriptor(model.bindingIr)))
		throw new TypeError("Native graph carriers differ from the compiled profile or public types");
...
export const createCompiledNativeModel = options => {
	...
	const exports = bindingIr.declarations.map(item => {
		...
		if(!source) throw new TypeError("Native graph export lacks a compiler-selected source declaration");
...
const carrierHeader = abi => {
	...
	const lines = ["#include <lean/lean.h>", "#include <stdint.h>", "LEAN_CASSERT(sizeof(size_t) * 8 == 64);"];
...
/**
 * Emit the selected native transport and ABI-check all carrier definitions with
