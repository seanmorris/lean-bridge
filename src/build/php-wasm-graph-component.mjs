...
export const generateCompiledPhpWasmGraph = (model, adapters) => {
	...
	const zend = generateCopiedPhpGraphZendAdapter(model.bindingIr);
	const transport = generateNativeCopiedGraphAdapters(model.bindingIr, abi, { initializer, wordBits: 32 });
	...
	if(zend[`include/${prefix}-graph-types.h`] !== transport.typesHeader
		|| manifest.layoutSha256 !== sha256(canonicalJson(transport.layout))) throw new Error("PHP-Wasm Zend and Lean graph layouts differ");
...
`;
	return { files: { ...zend, "graph/transport.c": transport.source, "graph/runtime.c": lifecycle, "graph/allocation-guard.h": nativeAllocationGuardHeader }
		, manifest, zendManifestPath: "graph-zend-manifest.json"
		, sources: ["graph/transport.c", "graph/runtime.c", `extension/${manifest.extension}.c`]
