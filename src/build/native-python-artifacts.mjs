/**
 * Verify ordinary Python loader inputs against native compiler artifacts.
 *
 * @file
 */
...
export const ordinaryPythonEvidence = async ({ nativeRoot, runtimeRoot, adapterRoot }) => {
	...
	if(adapter.schemaVersion !== 1 || adapter.profile !== "native-library-v1" || adapter.runtimeIdentity !== identity
		...
		|| (await nativeArtifactPaths(adapterRoot)).some(path => path !== "native-c-adapter.json" && !Object.hasOwn(adapter.files, path))) throw new Error("Python C adapter differs from compiled component");
	if(copiedGraph) for(const [path, source] of Object.entries(nativeGraphProjectionSources(model, receipt)))
		if((await readFile(join(adapterRoot, path), "utf8")) !== source) throw new Error(`Generated Python graph adapter source differs: ${path}`);
