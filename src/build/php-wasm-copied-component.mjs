...
export const buildPhpWasmCopiedComponent = async options => {
	...
	return buildElaboratedComponent({ ...options, targets: ["php-wasm"]
		...
		, validateModel: model => { compileCopiedPhpModel(model.bindingIr, { integerBits: 32, compounds: false }); options.validateModel?.(model); }
		, compileComponent: async ({ staging, model, metadata, sourceIdentity, adapters, compileOrder, generatedC, lakeWorkspace }) => {
			...
			const manifest = JSON.parse(zend["copied-zend-manifest.json"]), { surface } = compileCopiedPhpModel(model.bindingIr, { integerBits: 32, compounds: false });
