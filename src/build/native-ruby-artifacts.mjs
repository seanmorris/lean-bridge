/**
 * Verify ordinary Ruby loader inputs against native compiler artifacts.
 *
 * @file
 */
...
import { compileCopiedRubyModel } from "../backends/ruby/copied-model.mjs";
import { compileCopiedRubyGraphPackageModel, rubyGraphClearSource } from "../backends/ruby/copied-graph-package.mjs";
...
/**
 * Derive loader identities from closed native component and adapter inventories.
 *
...
export const ordinaryRubyEvidence = async ({ nativeRoot, runtimeRoot, adapterRoot }) => {
	...
	const projection = model.copiedGraph ? compileCopiedRubyGraphPackageModel(model.bindingIr) : compileCopiedRubyModel(model.bindingIr);
	...
	if(adapter.schemaVersion !== 1 || adapter.profile !== "native-library-v1" || adapter.runtimeIdentity !== identity
		...
		|| (await nativeArtifactPaths(adapterRoot)).some(path => path !== "native-c-adapter.json" && !Object.hasOwn(adapter.files, path))) throw new Error("Ruby C adapter differs from compiled component");
	if(copiedGraph) for(const [path, source] of Object.entries({ ...nativeGraphProjectionSources(model, receipt), "src/ruby-graph-clear.c": rubyGraphClearSource(prefix) }))
		if((await readFile(join(adapterRoot, path), "utf8")) !== source) throw new Error(`Generated Ruby graph adapter source differs: ${path}`);
