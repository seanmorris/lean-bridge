...
Fresh Lean interfaces supply names, types, documentation, source positions, effects, and theorem references. Cached `.ilean` files and source-scanned signatures supply no semantic evidence. During analysis, an explicit reviewed Binding IR bypasses compilation and is labeled `existing-validated`. Building that review requires [explicit source modules and fresh compiler reconciliation](../../docs/lean/existing-package.md#compile-a-reviewed-contract). npm accepts reviewed primitive signatures; native and PHP-Wasm also accept copied arrays and records. CPAN additionally accepts reviewed synchronous callbacks and returned closures with primitive arguments and results.
...
| [`package-license.mjs`](package-license.mjs) | Parses bounded SPDX expressions offline using pinned [identifier data](spdx-data.json); validates custom license paths and projects limited ecosystem vocabularies. |
