...
const scanLeanSource = (source, path) => {
  ...
  for (let index = 0; index < lines.length; index += 1) {
    ...
    while (
      ...
      chunks.length < 40 &&
      (balance > 0 || (bodyRequired
        ? !/(?:\:=|\bwhere\b)/.test(chunks.join(" "))
        : !/:/.test(chunks.join(" "))))
    ) {
...
const packageFacts = async root => {
  ...
  let npm = null;
  try { npm = packageJson === null ? null : JSON.parse(packageJson); } catch { npm = null; }
...
export const analyzeLeanProject = async (projectRoot, { signal = undefined } = {}) => {
  ...
  for (const declaration of declarations) {
    ...
    if (declaration.documentation === null) {
      diagnostics.push({
        ...
        message: `${declaration.fullName} has no documentation comment`, path: `${declaration.path}:${declaration.line}`, hint: null,
      });
    }
    for (const reason of reasons.filter(reason => new Set([
      "foreign-contract-required", "effect-adapter-required", "callable-projection-required",
      "unsupported-parameter-type", "unsupported-result-type", "implicit-or-instance-parameter",
  ...
  if (facts.version === "0.0.0-local") {
    diagnostics.push({
      ...
      path: facts.lakefile, hint: "Set version in lakefile.toml or package.json before release.",
    });
  }
  if (bindingIr === null) {
    diagnostics.push({
      ...
      path: null, hint: "Resolve the reported adapter hints or select an existing Binding IR.",
    });
  }
  ...
  return Object.freeze({
    ...
    declarations: Object.freeze(declarations.map(item => ({
      ...
      documented: item.documentation !== null, foreign: item.externSymbol !== null,
