...
/**
 * Maps each closed CLI result status to its stable process exit code.
 *
 * @param status - Validated CLI status returned by a command result.
 */
...
/**
 * Reports CLI contract failures with stable machine-readable codes and structured diagnostic context.
 */
export class CliContractError extends Error
{
	/**
   * Initializes the error used to report CLI contract failures, preserving its code, message, and diagnostic context.
   *
...
/**
 * Reports CLI cancellation failures with stable machine-readable codes and structured diagnostic context.
 */
export class CliCancellationError extends Error
{
	/**
   * Initializes the error used to report CLI cancellation failures, preserving its code, message, and diagnostic context.
   *
   * @param message - Human-readable explanation of the failure.
   */
...
/**
 * Validates CLI config against its closed contract before it enters the machine-readable CLI contract.
 *
 * @param config - Parsed CLI configuration whose keys and values are checked against the closed command contract.
 */
 ...
 * @param root0 - Named inputs and dependency overrides used to parse CLI arguments.
...
/**
 * Validates CLI result against its closed contract before it enters the machine-readable CLI contract.
 *
...
/**
 * Creates an immutable machine-readable diagnostic with optional source path and remediation hint.
 *
 * @param root0 - Fields recorded in the diagnostic contract.
 * @param root0.code - Stable machine-readable code that identifies the failure category.
 * @param root0.severity - Closed severity level used by human and JSON renderers.
 * @param root0.message - Human-readable explanation of the failure.
 * @param root0.path - Optional source path associated with the diagnostic.
 * @param root0.hint - Optional actionable remediation shown to the caller.
 */
export const diagnostic = ({ code, severity = "error", message, path = null, hint = null }) =>
	Object.freeze({ code, severity, message, path, hint });

/**
 * Creates an immutable structured prompt with closed choices and optional declaration context.
 *
 * @param root0 - Fields recorded in the prompt contract.
 * @param root0.id - Stable prompt identifier used to correlate a response.
 * @param root0.message - Human-readable question presented to the caller.
 * @param root0.choices - Closed set of accepted response values.
 * @param root0.required - Whether the command must receive an answer before proceeding.
 * @param root0.declaration - Optional Lean declaration that caused the prompt.
 */
