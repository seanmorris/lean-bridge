...
`lean-bridge verify` uses [the verification handler](verify.mjs) without loading author handlers or publication adapters. It selects ecosystem-neutral package-set or local npm verification from the receipt kind, while complete signed options select the existing authenticated archive validator. Parsing bypasses project configuration; only output format and progress environment settings apply. Package-set verification requires the `.json.sha256` sidecar and hashes every named archive without extraction or execution. Results have no project, distinguish unsigned consistency from authenticated archives, and preserve the CLI's result envelope and exit codes. Failed signed verification never falls back to a local check.
