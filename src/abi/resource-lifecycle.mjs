/**
 * Implements the resource lifecycle module in the ABI subsystem.
 *
 * @file
 */
...
/**
 * Reports resource lifecycle generation failures with stable machine-readable codes and structured diagnostic context.
 */
export class ResourceLifecycleGenerationError extends Error
{
	/**
   * Initializes the error used to report resource lifecycle generation failures, preserving its code, message, and diagnostic context.
   *
...
const compileSite = (site, role, typeMap, parameters) => {
	...
	const plan = {
		...
		, transition: site.ownership === "copy" ? "copy" : site.ownership === "borrow" ? "borrow" : site.ownership === "lease" ? "acquire-lease" : "transfer"
	};
...
/**
 * Compiles resource lifecycle version 1 into the explicit representation consumed by the generated host-adapter ABI.
 *
